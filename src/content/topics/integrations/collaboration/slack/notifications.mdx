...
4. (Optional) You can further define your subscription by giving it a specific flag or flag regex, a status, an action, or a tag. You'll only receive updates for flags that match these parameters.

![The modal to select subscription parameters.](slack-subscription-parameter-select-modal.png)

4. Click **Preview** to view the list of flags that your parameters include. Your subscription will include any newly created flags that match the parameters.
...
For example, the following command will create a subscription for the 'chat-response' flag in the 'qa' environment of the 'chatbot' project.
```
