...
If you use a different JSON template than the one provided above, you must include the **`title`** and **`url`** fields or the trigger will not save useful data about the event in the flag's audit log. The **`url`** is the URl of your Datadog event.
