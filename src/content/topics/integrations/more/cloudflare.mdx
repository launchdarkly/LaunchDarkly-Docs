...
<Callout intent="info">
  ...
  The Cloudflare integration is only available to members of LaunchDarkly's Early Access Program (EAP). If you want access to this feature, [join the EAP](http://launchdarkly.com/eap).
...
![The LaunchDarky Cloudflare integration screen.](../../images/cloudflare-create-integration.png)
...
```
 ...
 To learn more about bootstrapping client side feature flags from the server, read [Bootstrapping from the server](/guides/platform-specific/static-sites#bootstrapping-from-the-server).