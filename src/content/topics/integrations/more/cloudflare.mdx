...
class FlagsStateInjector {
  async element(element) {
    ...
    const evalContext = { key: 'context-key-123abc' }
    const allFlags = await ldClient.allFlagsState(evalContext, { anonymous: true })
...
<!-- question about line 268 above: should it be anonymous: true, since client-side SDKs set key?  Also, do we need to specify a context kind? SC-164921 -->
