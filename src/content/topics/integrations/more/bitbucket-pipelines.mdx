...
3. Add two environment variables to your Pipelines settings:
   _ `LAUNCHDARKLY_ACCESS_TOKEN`: Copy your API access token from the [Authorizations tab](https://app.launchdarkly.com/settings/authorization).
   _ `LAUNCHDARKLY_PROJECT_KEY`: Copy your project key from your [LaunchDarkly Projects tab](https://app.launchdarkly.com/settings/projects).

![The Account Settings screen with the Access Tokens tab selected.](../../images/settings-access-tokens.png)
...
Be careful to use the LaunchDarkly **project key** in the Bitbucket Pipelines settings, not the environment key. The
screenshot below calls out both the project and environment keys.

  </CalloutDescription>
</Callout>

![The Projects screen with the project key and environment keys called out.](../../images/settings-projects-keys-callout.png)
