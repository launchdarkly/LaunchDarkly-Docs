...
* Have a LaunchDarkly API access token for use with Compass. To learn more, read [Creating API access tokens](/home/account-security/api-access-tokens#creating-api-access-tokens).
...
3. In the top navigation, click the **Apps** link.
4. Find **LaunchDarkly** in the list of apps and click **Install**.
5. After the LaunchDakly app is installed, click **Configure**.
6. Click **Allow access** to approve the access to your Atlassian account requested by the LaunchDarkly app.
7. On the **LaunchDarkly is requesting Access** screen, review the requested permissions, then click **Accept**.
...
8. On the app configuration screen, enter your LaunchDarkly API access token and click **Submit**.
...
Now that you've set up the Compass integration for Compass, you must connect your LaunchDarkly projects to Compass components to see feature flag data in Compass.
...
2. Copy the page URL, for example, `https://app.launchdarkly.com/starship-command/production/features`, to your clipboard. Note that the URL contains the project name and environment.
...
3. In the **Other links** section of your Compass component, remove the LaunchDarkly link.
