...
The Compass integration requires a LaunchDarkly personal or service API access token with a role policy that allows the token to perform the `createIntegration` and `deleteIntegration` actions. 
...
5. Click the **Advanced editor** button and specify the following policy in the **Editor** field to allow the token to perform the `createIntegration` and `deleteIntegration` actions on integration resources:
