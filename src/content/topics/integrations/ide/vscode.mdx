...
You can create a new access token on the **Account settings** page, on the [**Authorization** tab](https://app.launchdarkly.com/settings/authorization). The access token must have sufficient permissions to support the extension's functionality. To learn more, read [Creating API access tokens](/home/account-security/api-access-tokens#creating-api-access-tokens).
