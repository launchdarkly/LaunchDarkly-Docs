...
The integration only sends `flag` actions to Dynatrace. To learn more, read [Feature flag actions](/home/account-security/custom-roles/actions#feature-flags-actions).
...
6. If this is all the configuration you need to do, click **Save Dynatrace Configuration**.
...
import {CodeTabs} from '../../../components/mdx/code/codeTabs';
import {CodeTabItem} from '../../../components/mdx/code/codeTabItem';
...
To learn more about the Resource configuration fields, read [Resources in custom roles](/home/account-security/custom-roles/resources).
...
To learn more about using the policy editor, read [Policies in custom roles](/home/account-security/custom-roles/policies).
...
<Callout intent="info">
<CalloutTitle>The integration does not support all Dynatrace entities</CalloutTitle>
<CalloutDescription>

The integration only attaches events to **Application** entities within Dynatrace.
...
4. In the "Select a property type..." dropdown, select **New**.
5. Enter `Dynatrace` in the "Name" field. The _Key_ field auto-populates with the string `dynatrace`.
6. Enter the name of the tag you created in the "Value" field.
