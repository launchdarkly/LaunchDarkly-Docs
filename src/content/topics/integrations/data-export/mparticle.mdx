...
- google
 ...
 Anonymous user identifiers let you use an `alias` event to  connect two user IDs that represent the same person. To learn more about `alias` events, read [Associating anonymous users with logged in users](/home/managing-users/anonymous-users#associating-anonymous-users-with-logged-in-users).
