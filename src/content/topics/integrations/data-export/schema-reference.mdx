...
Alias events are produced when the client application calls an SDK method called
`Alias`. They are also emitted automatically by clientside SDKs when a user
identifies as a new user after previously identifying as an anonymous user.

<Callout intent="alert">
<CalloutTitle>mParticle schema differences</CalloutTitle>

<CalloutDescription>

Alias events are not sent to mParticle destinations
