...
However, by default, the React SDK camel-cases all flag keys for easier access with dot notation. To use the code references feature in conjunction with the React SDK, [configure `ld-find-code-refs` to generate camel-case aliases for the flag keys](https://github.com/launchdarkly/ld-find-code-refs/blob/master/docs/ALIASES.md#flag-keys-transposed-to-common-casing-conventions) or configure the React SDK to disable this camel-casing feature.
