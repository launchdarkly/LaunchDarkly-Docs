...
<Callout intent="info">
<CalloutTitle>Approvals are an Enterprise feature</CalloutTitle>
<CalloutDescription>

This feature is only available for customers on a LaunchDarkly Enterprise plan. To enable approvals on your account, [contact Sales](https://launchdarkly.com/contact-sales/).
...
![LaunchDarkly's ServiceNow OAuth provider registration form.](../../images/servicenow-register-oauth-launchdarkly.png)
...
![The ServiceNow external client endpoint registration form with the Redirect URL configured.](../../images/servicenow-register-oauth-provider.png)

9. In LaunchDarkly's **OAuth provider registration** modal, enter your ServiceNow subdomain. For example, if your ServiceNow instance is hosted at 'https://launchdarkly.service-now.com', your subdomain is 'launchdarkly'.
...
2. Click the three-dot **overflow menu** on the environment you wish to configure.
...
![LaunchDarkly environment approval settings with ServiceNow configured.](../../images/servicenow-environment-approval-settings.png)
...
1. Create a new [Workflow Trigger](https://docs.servicenow.com/bundle/istanbul-security-management/page/product/security-operations-common/task/create-workflow-triggers.html) for record "Created" on the "Change Request [change_request]" table.
...
![A configured Workflow Trigger in the ServiceNow Flow Designer.](../../images/servicenow-workflow-trigger.png)

3. Add a new "Ask For Approval" Action on your Trigger Change Request Record and set the approval rules.

![A ServiceNow "Ask For Approval" action configuration.](../../images/servicenow-manual-approval.png)

4. Add an `if` condition that requires Approval State `is` "Approved".

![A ServiceNow if condition configured for change request approvals.](../../images/servicenow-manual-approval-if.png)
...
![A ServiceNow update change request record action that moves the "State" to "Scheduled".](../../images/servicenow-if-update-1.png)
...
![A ServiceNow update change request record action that moves the "State" to "Canceled".](../../images/servicenow-else-update.png)

You can now request manual approvals in ServiceNow for flag changes in LaunchDarkly.

![A complete ServiceNow workflow for LaunchDarkly manual flag approvals.](../../images/servicenow-completed-manual-flow.png)
...
![A Workflow Trigger in the ServiceNow Flow Designer.](../../images/servicenow-workflow-trigger.png)
...
![A ServiceNow update change request record action that moves the "State" to "Implemented".](../../images/servicenow-if-update-2.png)

You can now track your LaunchDarkly flag changes in ServiceNow.

![A completed ServiceNow workflow for LaunchDarkly "automatic" approvals.](../../images/servicenow-automatic-approvals.png)
