...
## Filtering for feature flags and more with the menu
...
You can use the four types of filters in any combination to view different flags. If you click **Filter** without specifying any of the fields, all flags and resources will display.
...
You can also use in-line commands to filter flags and resources in Slack.
...
- **Add filter to Channel** adds the filter criteria to be added to the channel. This automatically adds flags and resources that match that criteria for this filter to the channel. A notification appears in the channel every time the status of one of the results changes.
- **Adjust Filter** opens the Filter screen where you can adjust the filtering criteria.
...
When a filter is applied to the channel, any sets of resources that match the filter criteria automatically send notifications to that channel.
