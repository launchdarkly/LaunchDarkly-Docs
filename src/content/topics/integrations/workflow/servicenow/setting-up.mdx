...
title: Setting up the ServiceNow approvals integration
...
- You must have a ServiceNow Client ID. To learn how to create these, search for "Get the Client ID and Client Secret" in [ServiceNow's documentation](https://docs.servicenow.com/) and choose the article for your ServiceNow version.
- You must have a secret for an OAuth API endpoint for external clients. To learn how to create these, search for "Create an OAuth JWT API endpoint for external clients" in [ServiceNow's documentation](https://docs.servicenow.com/) and choose the article for your ServiceNow version.
- If your ServiceNow instance has IP address access control enabled, you must create **inbound** ServiceNow IP access control rules corresponding to the the `outboundAddresses` from LaunchDarkly's [public IP list](/home/advanced/public-ip-list). To learn how to create IP access control rules, search for "IP Address Access Control" in [ServiceNow's documentation](https://docs.servicenow.com/) and choose the article for your ServiceNow version.
...
7. Set the **Redirect URL** to the redirect URL saved to your clipboard in step 2. Ensure you are following the directions for your ServiceNow version.
...
8. In LaunchDarkly's **OAuth provider registration** dialog, enter your ServiceNow host URL. For example, if your ServiceNow instance is hosted at `https://launchdarkly.service-now.com`, enter `https://launchdarkly.service-now.com`.
9. Enter the **Client ID** and **Client Secret** for the client you created in step 5.
10. Click **Register OAuth Provider**.
11. Click **Allow**. ServiceNow may prompt you to log in to your ServiceNow account at this step.
...
8. (Optional) Fill in the **Detailed information column name** field with the name of the ServiceNow Change Request column you would like LaunchDarkly to populate with detailed approval request information. This field defaults to `justification` if you do not provide a value.
9. Click **Save settings**.

![LaunchDarkly environment approval settings with ServiceNow configured.](servicenow-environment-approval-settings.png)<br />
...
To learn more, search for "Flow Designer" and "Workflow Editor" in [ServiceNow's documentation](https://docs.servicenow.com/) and choose the articles for your ServiceNow version.
...
1. Create a new Workflow Trigger for record "Created" on the "Change Request [change_request]" table.  To learn how to do this, search for "Create Security Operations workflow triggers" in [ServiceNow's documentation](https://docs.servicenow.com/) and choose the article for your ServiceNow version.
