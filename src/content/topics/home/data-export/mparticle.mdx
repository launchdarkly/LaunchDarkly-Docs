...
10. (Optional) If you use anonymous contexts, select the **Anonymous user identifier** type from the menu.

 If you select this user type, you must also configure your mParticle SDK to send an `alias` event. `Alias` events connect an anonymous user ID with the user ID of a known user in mParticle. This ensures that your users are identified and counted accurately. To learn more about `alias` events in mParticle, read [mParticle's documentation on associating anonymous and known users](https://docs.mparticle.com/guides/idsync/aliasing/).
