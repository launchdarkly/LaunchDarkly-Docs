...
An experiment's randomization unit is the context kind the experiment uses to assign traffic to each of its variations. For example, if you choose the context kind `user` as an experiment's randomization unit, then the experiment will divide contexts into the experiment's variations by individual users. User Sandy might be assigned to variation A and user Jesse assigned to variation B. However, if you choose `organization` as a randomization unit, then the experiment will divide contexts into the experiment's variations by organizations instead.
