...
CSS selectors designate specific areas of your product UI, like buttons or images, that your users can interact with. You can create a metric that monitors interaction with one or more CSS selectors.
...
The CSS selector must be present in your application after the end user interacts with it in order for LaunchDarkly to record the metric. For example, if the user clicks an element that is dynamically removed from your application in response to the click, then LaunchDarkly will not record the metric event.
