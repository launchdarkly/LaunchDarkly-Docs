...
Your anonymous contexts must have their own unique context keys to use Experimentation. To learn more, read [Anonymous contexts and users](/sdk/features/anonymous).
...
8. (Optional) Choose up to five context attributes to slice results by. To learn more, read [Slicing results](/home/analyzing-experiments/results#slicing-results-by-attribute).
...
9. Choose a boolean, string, or number flag to use in the experiment from the **Select flag** menu. You cannot run experiments on JSON flags.
...
For an experiment to begin recording data, the flag used in the experiment must be on. To learn how, read [Turning flags on and off](/home/getting-started/toggle).
