...
SSO allows your team to authenticate with LaunchDarkly using the same identity provider (IdP) you use for your other internal and external services. LaunchDarkly implements SSO with the SAML 2.0 protocol. Administrators can use SSO with their IdP to manage access rights in LaunchDarkly. After SSO is turned on, administrators can also configure System for Cross-domain Identity Management (SCIM) provisioning through their IdP.

Once SCIM is configured, administrators can optionally turn on and configure team sync with IdP. Team sync with IdP allows administrators to sync groups in their IdP with teams in LaunchDarkly to automate team member provisioning.
...
This table lists the external IdPs for which we provide configuration guidance, as well as their supported features:
...
When you're satisfied with the SSO integration and are ready to turn it on for all account members in LaunchDarkly, turn on SSO by following the procedure below.
...
Once SSO is turned on, the LaunchDarkly login procedure defers to your identity provider for authentication. Users will no longer be able to log in with their existing LaunchDarkly password, and users logging in to LaunchDarkly for the first time will need to log in through their IdP.
...
You can turn off SSO at any time. When SSO is turned off, any existing members will still be able to sign into LaunchDarkly with their previous passwords, or reset their passwords.
...
<Callout>
<CalloutTitle>Provisioning with SCIM is an Enterprise feature</CalloutTitle>
<CalloutDescription>

    SCIM is only available to customers on an Enterprise plan. To learn more, [read about our pricing](https://launchdarkly.com/pricing/). To upgrade your plan, [contact Sales](https://launchdarkly.com/contact-sales/).

    Additionally, You must be a LaunchDarkly administrator and have SSO enabled to complete the following procedure.

</CalloutDescription>
</Callout>

If you have turned on SSO, you can configure SCIM provisioning as well. SCIM facilitates _real-time user provisioning_, which means your IdP can use it to create, update, and deactivate members in LaunchDarkly prior to user authentication.
...
You may only have one SCIM provider linked to your IdP at once. If you need to change providers, disconnect the original provider by following the workflow below and configure a different one through your IdP.
...
<Callout intent="primary">
<CalloutTitle>Team sync with IdP is an Enterprise feature</CalloutTitle>
<CalloutDescription>

Team sync with IdP is only available to customers on an Enterprise plan, and is currently only supported by the Okta integration. To learn more, [read about our pricing](https://launchdarkly.com/pricing/). To upgrade your plan, [contact Sales](https://launchdarkly.com/contact-sales/).

</CalloutDescription>
</Callout>

LaunchDarkly allows you to sync groups from your IdP to teams within your account. This feature will create a new team, or link an existing one, with the same name and membership as the IdP group.

Once team sync is turned on:
* New teams can only be created through Group Push in Okta, and not from the LaunchDarkly UI.
* Changes to your synced teams' name and memberships can only be made through your IdP. Unsynced teams can still be maintained in the LaunchDarkly UI.
* Team permissions will still be managed in LaunchDarkly. To learn how to add custom roles to a team, read [Managing team permissions](/home/teams/managing#managing-team-permissions).

To turn on team sync in LaunchDarkly:

1. Log in to LaunchDarkly as an administrator or account owner.
2. Navigate to the "SSO management" section of the [**Security**](https://app.launchdarkly.com/settings/security) tab on the **Account settings** page. Please note that SCIM must be configured in order for this feature to be turned on. 

3. Click **Turn on team sync**. A confirmation modal will appear that confirms that existing teams will not be deleted, and that you will have limited team management permissions in LaunchDarkly for synced teams.

   ![Turn on team sync within "SSO management" section of the "Security tab".](settings-security-turn-on-team-sync.png)

5. Teams are now able to be managed by your IdP, which is done within the admin panel of the IdP. We currently only support team sync for the following identity providers:
   * [Okta](/home/account-security/okta)
   * Azure (coming soon)


6. Teams that have been synced with an IdP now have limited team management permissions in LaunchDarkly. This new team sync status will be denoted by either a **Synced** or **Not Synced** indicator.
   ![Teams synced with IdP.](teams-synced-with-idp-PLACEHOLDER.png)
