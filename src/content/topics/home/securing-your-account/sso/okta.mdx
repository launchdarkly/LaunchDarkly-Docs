...
However, it is important to note that using the same Okta group for assignments and for Group Push is not supported. To maintain consistent group membership between Okta and LaunchDarkly, you need to create a separate group that is configured to push teams. This is a known limitation of Okta's Group Push feature.
