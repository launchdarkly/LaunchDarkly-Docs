...
To push an existing Okta group to create a new team in LaunchDarkly:
...
5. To link an Okta group to an existing LaunchDarkly team, after clicking "Push Groups" and choosing to find a group by name or by rule and finding the group you wish to link, Okta will attempt to find a matching team based on the group name to link. Otherwise, if the team is not automatically selected based off of the team name, click the dropdown under "Create Group", select the "Link Group", and search for the team you wish you link.
