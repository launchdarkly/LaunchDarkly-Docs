...
You can use Okta to create new teams in LaunchDarkly, or link an Okta group to an existing team, in order to maintain team memberships within Okta.

Key requirements:
- You must have SCIM provisioning enabled in the LaunchDarkly Okta app. If it is not enabled, you will be prompted to do so.
- Any group members that you want to push to a team in LaunchDarkly MUST be previously provisioned and assigned to the LaunchDarkly application in Okta. Once a group is synced with a LaunchDarkly team, changes should never be made from the LaunchDarkly UI.

It is also worth noting that using the same Okta group for assignments and for Group Push is not supported. To maintain consistent group membership between Okta and LaunchDarkly, you need to create a separate group that is configured to push teams. This is a known limitation of Okta's Group Push feature.

To learn more about Okta's Group Push feature, [read more here](https://help.okta.com/en-us/Content/Topics/users-groups-profiles/usgp-about-group-push.htm).
...
Once the push status in Okta is "Active", you can confirm the new team was created and synced with Okta on the **Teams** tab in LaunchDarkly.

![The "Teams" tab showing teams synced with Okta.](teams-synced-with-idp.png)

### Link an Okta group to an existing team

LaunchDarkly also supports [Group Linking with Okta](https://help.okta.com/en-us/Content/Topics/users-groups-profiles/usgp-enhanced-group-push-main.htm), which adds the ability to push a group name that already exists within the LaunchDarkly.
...
![The "Teams" tab showing teams synced with Okta.](okta-group-link.png)
...
Once the push status in Okta is "Active," you can confirm that the team membership matches the group membership in Okta on the LaunchDarkly **Account settings** page.

![Teams synced with IdP.](teams-synced-with-idp.png)

### Limitations of team sync with Okta

Once a team has been synced to an Okta group, it is unable to be unsynced. The only way to remove the team is to unlink the pushed group in Okta.

![Unlink and deactivate group push in Okta.](okta-deactivate-groups.png)

After clicking on the group's push status, choose the "Unlink pushed group" option and then choose to "Delete the group in the target app".

![Unlink pushed group in Okta.](okta-unlink-push-group.png)

Deactivating group push or unlinking a group while leaving the group in the target app will result in an orphaned team in LaunchDarkly that cannot be managed or maintained within the LaunchDarkly UI. To remove the team, simply re-sync the team [using the steps above](/home/account-security/okta#link-an-okta-group-to-an-existing-team) and then choose the "Unlink pushed group" & "Delete the group in the target app" options.