...
- Any group members that you want to push to a team in LaunchDarkly MUST be previously provisioned and assigned to the LaunchDarkly application in Okta. Once a group is synced with a LaunchDarkly team, team membership changes will be disabled in the LaunchDarkly UI.
- Using the same Okta group for assignments and for Group Push is not supported.
- To maintain consistent group membership between Okta and LaunchDarkly, you need to create a separate group that is configured to push teams. This is a known limitation of Okta's Group Push feature.
