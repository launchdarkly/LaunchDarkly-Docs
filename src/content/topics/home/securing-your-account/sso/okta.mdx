...
Deactivating group push or unlinking a group while leaving the group in the target app will result in an orphaned team in LaunchDarkly that cannot be managed or maintained within the LaunchDarkly UI. To undo this action, simply re-sync the team using the steps above and then choose the "Unlink pushed group" & "Delete the group in the target app" options.