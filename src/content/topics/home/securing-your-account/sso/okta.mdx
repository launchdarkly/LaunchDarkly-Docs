...
- To maintain consistent group membership between Okta and LaunchDarkly, create a separate group that is configured to push teams. This is a known limitation of Okta's Group Push feature.
...
After the push status in Okta is "Active," you can confirm that the team membership matches the group membership in Okta on the LaunchDarkly **Account settings** page.
...
Deactivating group push or unlinking a group while leaving the group in LaunchDarkly results in an orphaned team. You will not be able to manage or maintain an orphaned team within the LaunchDarkly UI. To remove the team, first [re-link the team](#link-an-okta-group-to-an-existing-team), choose the "Unlink pushed group" option, then choose the "Delete the group in the target app" option.