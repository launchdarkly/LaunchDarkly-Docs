...
You may only have one IdP linked to your LaunchDarkly account at once. If you need to change providers, disconnect the original provider by following the workflow below and enable a different one through your IdP.
...
Please note that it is recommended to use either SCIM-based OR SAML-based member role and team assignment. If you use both SAML and SCIM methods in conjunction, team assignment set up with the SAML custom attribute (`teamKey`) will be overridden by SCIM team assignment.