...
![The "Enable multi-factor authentication" dialog with an obscured six digit code.](mfa-qr.png)
...
Admins can also see whether account members have MFA enabled on the individual member's **Settings** page. If a member does not have MFA enabled, admins can send an email requesting that the account member enable MFA.
