...
When the Relay Proxy is in offline mode, it does not send events to LaunchDarkly, and event-driven app features do not indicate new activity. For example, flag statuses and the flag evaluations graph do not update to reflect evaluations from an offline Relay Proxy. To learn more, read [Events in offline mode](https://github.com/launchdarkly/ld-relay/blob/v7/docs/events.md#events-in-offline-mode).
