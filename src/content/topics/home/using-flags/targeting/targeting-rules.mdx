...
![A targeting rule that serves "true" to all user contexts that contain an "email" attribute with a value that ends in ".edu".](/static/flag-targeting-rule.png)
...
8. (Optional) If you need to undo the changes you just made, click the **undo arrow** icon to discard all changes:
  ...
  ![A feature flag's "discard all changes" button called out.](/static/flag-targeting-discard-changes.png)
