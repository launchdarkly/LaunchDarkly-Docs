...
published: true
tags: ['target', 'user', 'flag', 'export', 'csv', 'enterprise']
---
...
Very large lists of named users increase the size of the payload the SDK fetches when initializing, as well as the memory footprint of the SDK. If you use Big Segments, you may have lists of over 15,000 users, which can increase initialization time. To learn more, read [User segments](/home/users/segments).
...
2. In the "Individual targeting" section, click the **overflow menu**.
...
If you want to target the same large numbers of users in multiple flags, you can create a segment, set up targeting rules for the segment, and target the segment instead. To learn how, read [User segments](/home/users/segments).
