...
Enterprise customers using [Teams](/home/teams) can assign a team as the flag's maintainer. This offers the additional benefit of shared accountability and also ensures that if one member leaves the team, the flag still has other maintainers.
...
![The flag's maintainer and "pencil" icon.](edit-flag-maintainer.png)

### Name, description

You can change the flag's name and description here. The description can include Markdown and inline Confluence links. To learn more about the LaunchDarkly's integration with Confluence read [Confluence embedded pages](/integrations/confluence).
...
If you're using a client-side or mobile SDK, you must expose your feature flags in order for the client-side or mobile SDKs to evaluate them. To learn more, read [Making flags available to client-side and mobile SDKs](/home/flags/feature-flags#making-flags-available-to-client-side-and-mobile-sdks).
...
You can create new flags by cloning existing flags. When you clone a flag, LaunchDarkly makes a new flag and copies the original flag's targeting configuration for all environments. To learn more, read [Cloning flags](/home/flags/feature-flags#cloning-flags).
