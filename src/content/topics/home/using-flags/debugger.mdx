...
If your environment has not registered any events, the status updates to indicate that. If you see this status frequently, it may mean that have configured your SDK incorrectly. To learn more, read [SDK concepts](/sdk/concepts).

![Flag events connected to live event stream, but with no recent evaluations.
](debugger-connection-never-seen-events-state.png)
...
![Error connecting to the event steam.
](debugger-connection-error-state.png)
