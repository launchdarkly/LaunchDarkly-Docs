...
The debugger gives you real-time insight into the events your application is sending to LaunchDarkly, so you can ensure that you've set up LaunchDarkly correctly. Different types of events appear, in detail or in summary, depending on what you want to view.
...
### The debugger connection state

The connection state for the "Flag events" tab indicates if events have occurred in your environment recently.
...
The debugger displays evaluation counts and variation breakdowns for each flag evaluated since you opened the debugger. Click on a flag or select it from the menu to access individual event information. 

Flag evaluation events display as a summary. To get detailed evaluation event information for a flag, select it and click **Debug**. This temporarily enables full-fidelity debug events for the flag. To learn more, read [Event kinds](/home/data-export/schema-reference#event-kinds).

Here is an image of the **Debug** button for a flag:

![A flag with the "Debug" button called out.](debugger-flag-debug-button-callout.png)

View additional information, including the user attributes used for evaluation, by clicking on an individual event.

<Callout intent="info">
<CalloutTitle>You must have permission to use the Debug feature</CalloutTitle>
<CalloutDescription>

To use the **Debug** button, you must either have a built-in Writer role or a custom role that allows the <code>updateOn</code> action on the <code>flag</code> resource. To learn more, read [Custom roles](/home/members/custom-roles) and [Feature flag actions](/home/members/role-actions#feature-flag-actions).
...
Only events with matching attribute values now display on the **Debugger** list.

To add additional attribute values to your filter, click on the attribute name and select the additional values from the menu. The list displays only events that match all of the attribute values you selected:
...
Only detailed events corresponding to the selected variation now appear in the list. You can also filter by multiple variations.
...
The Experimentation events tab only displays for customers who have Experimentation enabled. To learn more, read [About Experimentation](/home/about-experimentation).
