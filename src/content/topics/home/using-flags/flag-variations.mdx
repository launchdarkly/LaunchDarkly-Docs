...
- **Boolean** flags have two settings: `true` or `false`. On the Variations tab you can edit the name and description for the `true` and `false` variations, and change the default variations for new environments.
- **Multivariate** flags have multiple settings that let you define different variations including string, number, or JSON. On the Variations tab you can edit any variation’s value, name, and description. You can also add and delete variations for a multivariate flag. To learn more, read [Understanding multivariate flags](#understanding-multivariate-flags).
...
Multivariate flag variation options include string, number, or JSON. JSON values can be no longer than 16 digits.
