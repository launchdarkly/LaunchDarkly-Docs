...
Boolean flags don't use variations, because they can be set to only `true` or `false`. Multivariate flags, however, can be configured in many ways to create custom experiences or reactions.

Here are the flag modifications you can do in the flag's Variations tab:

- **Boolean flags**: you can edit each variation’s name and description, and change the default variations for new environments.
...
Multivariate flags let you use one flag to serve multiple variations of a feature simultaneously. Flag settings include strings, numbers, JSON objects, or JSON arrays. You can use these for A/B/n testing.
