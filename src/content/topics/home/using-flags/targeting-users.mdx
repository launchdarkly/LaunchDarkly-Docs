...
Here, we've created a custom rule with two conditions. This rule serves `true` to users who are not in the segment `Test Segment` and whose `Account` attribute does not start with `test`.
