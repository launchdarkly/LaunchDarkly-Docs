...
This topic explains how to use a flag's **Targeting** tab to control which users receive a variation of a feature flag. Configuring which users receive which variation of a flag is referred to as a flag's targeting.
...
In this example, you will turn on a feature for a group of external beta testers. First, ensure that your backend sends a custom attribute called `groups` that identifies users that should be in the `beta_testers` group.
...
Any users in the `beta_testers` group now receive the new dashboard feature. You have also kept the feature on for your internal tester Ernesto, since there's still an Include rule for him.
