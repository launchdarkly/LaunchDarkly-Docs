...
When the toggle is turned off, LaunchDarkly will serve the **Off** variation for your feature flag. This is sometimes referred to as the fallback value. For boolean flags, the **Off** variation is set to `false` by default. For multivariate flags, you select one of your custom variations. You can customize the **Off** variation for both boolean and multivariate flags in the **Targeting** tab.

If you do not specify an **Off** variation, then LaunchDarkly will return the fallback variation defined in your code.

<Callout intent="info">
<CalloutTitle>Setting a fallthrough value in your SDK call</CalloutTitle>
<CalloutDescription>

Regardless of how you set up targeting in your flag dashboard, each time you evaluate a flag from the SDK, you must include a fallthrough value as one of the parameters. This fallthrough value is the value served to the user if the SDK cannot connect to LaunchDarkly. To learn more, read [Evaluating flags](/sdk/features/evaluating).
