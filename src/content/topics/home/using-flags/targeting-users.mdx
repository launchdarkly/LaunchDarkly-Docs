...
Here, we've created a custom rule with two conditions. This rule serves `true` to all "Not test users" who are not in "Test Segment" and their `account` attribute does not start with `test`.
