...
If a member belongs to one or more teams, a member's role(s), which includes either a built-in role or custom role(s), will aggregate with the custom role(s) that are applied to their team(s). Note that adding a member with a built-in Admin role will not affect their permissions since those roles can perform all actions in LaunchDarkly.
...
## Managing teams with your Identity Provider
