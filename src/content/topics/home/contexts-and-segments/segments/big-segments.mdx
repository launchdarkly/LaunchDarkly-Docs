...
2. In the **Included targets** search box, enter the key of the context you want to add or remove. If the context is present in the segment, a **Remove user** button appears. If the user is not present in the segment, an **Add user** button appears.
<!-- cSpell:disable TKTK the UI button hasn't been updated yet to say "Add/remove contexts" instead of users. We should re-check the UI  before GA launch. cSpell:enable -->

![Adding an individual context to a Big Segment.](big-segment-add-individual-user.png)
