---
path: /home/contexts/synced-segments
title: Syncing Big Segments from Amplitude cohorts
description: This topic explains how to create Big Segments that sync with an Amplitude cohort.
published: true
tags: ['amplitude', 'big segment', 'context', 'target', 'user']
---
...
The [prerequisites](/home/contexts/big-segments#prerequisites) for Big Segments apply to syncing segments.
...
- You must have a LaunchDarkly client-side ID for the environment you want to connect to Amplitude. To learn more, read [Copying your LaunchDarkly client-side ID](/home/creating-experiments/segment#copying-your-launchdarkly-client-side-id).
- If you are using server-side SDKs, or client-side SDKs with the Relay Proxy, there are [additional prerequisites](/home/contexts/big-segments#prerequisites).
...
![An access token with the Writer role called out.](create-access-token-callout.png)<br/>
...
<CodeSample>
<CSTab label="Example token policy">
...
Users in Amplitude must have a value in the field you map to the LaunchDarkly User Key field, typically the Amplitude user ID, to be included in a LaunchDarkly segment. The value in the LaunchDarkly User Key field becomes the context key in LaunchDarkly. Any Amplitude users missing a value in the mapped field are not included in the LaunchDarkly segment. To learn how to view the contexts in a segment synced with Amplitude, read [Viewing synced Amplitude cohorts in LaunchDarkly](#viewing-synced-amplitude-cohorts-in-launchdarkly).
