...
## Changes to the LaunchDarkly billing model

<Callout intent="alert">
<CalloutTitle>LaunchDarkly's billing calculations changed on April 1, 2023</CalloutTitle>
<CalloutDescription>

Your billing model may have changed from MAU to MCI on April 1, 2023. If you are unsure about how you are being billed, [contact Sales](https://launchdarkly.com/contact-sales/) or your LaunchDarkly Account Executive.

</CalloutDescription>
</Callout>

On April 1, 2023, LaunchDarkly introduced a new format for calculating billing and usage: monthly context instances (MCI). MCI replaces monthly active users (MAU). We provide documentation about MCI in this topic to help you understand any changes to your billing, but this change may not affect you before your plan renews.
...
- **Overview**: displays how many seats, client-side MAU, Experimentation keys or events, Data Export events, and server connection minutes your organization is using.
- **Client**: displays client-side monthly active user contexts and client connections. You can narrow the views by date range, SDK, and project/environment. This chart does not display any context kinds other than "user."
- **Experiments**: if you use Experimentation, displays Experimentation keys over the selected time or date range. You can narrow the views by context kind, project/environment, or date range. Depending on your billing model, you may be billed by Experimentation events, not keys. To learn more about pricing, read [Experiment pricing](/home/about-experimentation#experiment-pricing).
- **Data Export**: if you use Data Export, displays events published in the current calendar month.
- **Server**: displays server connection minutes. You can narrow the views by project and environment. LaunchDarkly does not bill for server connection minutes.
...
- **Overview**: displays how many seats, client-side MAU, Experimentation keys or events, Data Export events, and server connection minutes your organization is using.
- **Client**: displays client-side monthly active user contexts and client connections. MAU includes user contexts that are both single contexts, and user contexts that are part of a multi-context. You can narrow the views by date range, SDK, and project/environment. This chart does not display any context kinds other than "user."
- **Experiments**: if you use Experimentation, displays Experimentation keys over the selected time or date range. You can narrow the views by context kind, project/environment, or date range. Depending on your billing model, you may be billed by Experimentation events, not keys. To learn more about pricing, read [Experiment pricing](/home/about-experimentation#experiment-pricing).
- **Data Export**: if you use Data Export, displays events published in the current calendar month.
- **Server**: displays server connection minutes. You can narrow the views by project and environment. LaunchDarkly does not bill for server connection minutes.
