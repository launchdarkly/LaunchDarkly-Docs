...
Server-side, client-side, and mobile LaunchDarkly SDKs send a `feature` conversion event when you call the SDK's variation method for a flag in an experiment. To learn more about the variation method, read [Evaluating flags](/sdk/features/evaluating).

In addition, the JavaScript SDK and JavaScript-based SDKs also send a `variation` evaluation event when you call the SDK's `allFlags` method, unless you configure it not to. The React Web SDK automatically calls `allFlags` when you initialize the SDK, and when a context receives a new variation for a flag. To learn more, read [Getting all flags](/sdk/features/all-flags).
