...
By connecting metrics you create to flags in your LaunchDarkly environment, you can measure the changes in your customer's behavior based on what flags they evaluate. This helps you make more informed decisions, so the features your development team ships align with your business objectives. To learn about Experimentation use cases, read [Example experiments](/guides/experimentation/example-experiments).
...
If an experiment tells you a feature has positive impact, you can roll that feature out to your entire user base to maximize its value. Alternatively, if you don't like the results you're getting from a feature, you can toggle the flag off and minimize its impact on your user base.
...
Experiments are billed monthly based on the number of unique context keys in each experiment. To learn more about context keys, read [Built-in attributes](/home/contexts/built-in-attributes).

Your billing month begins on the same day of the month your contract began with LaunchDarkly. For example, if your contract with LaunchDarkly began on January 15th, your Experimentation billing month resets the 15th of every month. To estimate your Experimentation billing costs, multiply the number of experiments you run each month by the number of unique context keys in your experiment audiences. If an experiment has multiple iterations, LaunchDarkly still counts it as one experiment. If the same context key appears in two different experiments, LaunchDarkly counts the key twice for billing purposes.

<Callout intent="info">
<CalloutTitle>You may be billed for experiment events instead of keys</CalloutTitle>
<CalloutDescription>

Customers on older contracts may be billed by Experimentation event count, rather than unique context keys. To learn more, read [Experimentation events](/home/about-experimentation/events). If you are unsure about how you are being billed for Experimentation, [contact Sales](https://launchdarkly.com/contact-sales/) or your LaunchDarkly Account Executive.
