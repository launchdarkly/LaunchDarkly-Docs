...
The LaunchDarkly sidebar has a dropdown menu that shows you the current project and environment and allows you to quickly switch between projects and environments.
...
Cloning an environment is currently only available as a feature through the REST API. To learn more, read [Create Environment](https://apidocs.launchdarkly.com/tag/Environments#operation/postEnvironment).
...
* Users in Segments, Amplitude synced segments, and Big segments do not carry over to the new environment. Instead, an empty segment is created as a placeholder for flag rules to reference. To learn more about how to sync these segments, read [Big Segments](/home/users-and-segments/big-segments.mdx).
