...
published: true
tags: ['environment', 'project', 'key', 'sdk key', 'mci', 'clone', 'ttl', 'mau']
---
...
Most customers have access to 300 environments per project by default. If you are on a **legacy Starter plan**, your account may be limited to two environments. To request an increase to your environment limit, [contact Support](https://support.launchdarkly.com/hc/en-us/requests/new).
...
![The "Environments" tab on the quick search bar with an environment name entered.](env-search-bar-open-env.png)

5. Click the environment's name.
...
## Finding and resetting an environment's SDK key, mobile key, or client-side ID

You can view and reset the SDK key, mobile key, and client-side ID for an environment from your **Account settings** page.

You need one of these keys when you set up your SDK. Which key you need depends on which SDK you use. To learn more about different SDK key types, read [Understanding the different types of SDKs](/sdk/concepts/client-side-server-side#understanding-the-different-types-of-sdks) and [Keys](/sdk/concepts/client-side-server-side#keys).
