...
concurrency:
  ...
  cancel-in-progress: true # cancel in-flight workflow run if another push was triggered
...
<Table>
  ...
  <TableBody>
    <TableRow>
      ...
      <TableCell>
        A LaunchDarkly access token with Writer-level access, or access to the <code>code-reference-repository</code>{' '}
        custom role resource. Supply this as a GitHub repository secret.
      </TableCell>
    ...
    <TableRow>
      ...
      <TableCell>
        Sets the repository name for this project. This is useful if you have a single monorepo containing multiple YAML
        configurations each mapping to its own <code>projKey</code>. Each YAML configuration must have a unique{' '}
        <code>LD_PROJ_KEY</code> and <code>LD_REPO_NAME</code> combination so it displays correctly in the flags list.
