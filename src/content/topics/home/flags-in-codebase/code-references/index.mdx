...
You can view existing code references for feature flags in the flag dashboard.

To view existing code references, visit a feature flag's **Code references** tab.
...
After you remove all code references mentioning that flag from the codebase and rerun the scanning tool, LaunchDarkly creates an Extinction event and notifies you that it has removed the flag from the codebase. You can also use the API to find these events. To learn more, read [List Extinctions](https://apidocs.launchdarkly.com/tag/Code-references#operation/getExtinctions).

![The "Code references" tab for an extinct feature flag.](flag-code-references-extinct.png)
...
To learn more about monorepo support, read [Projects](https://github.com/launchdarkly/ld-find-code-refs/blob/master/docs/CONFIGURATION.md#projects) in the `ld-find-code-refs` configuration documentation.
