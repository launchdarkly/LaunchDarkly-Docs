...
To use CircleCI Orbs, you must be using a Circle Workflow version of 2.1 or higher. If you're using an earlier version, try manually using the utility binary or docker image to create your own workflow job. To learn more, read <a href="https://github.com/launchdarkly/ld-find-code-refs/tree/master/README.md#execution-via-cli">Execution via CLI</a> in the `ld-find-code-refs` documentation.
...
To skip searching files and directories, use an [`.ldignore`](https://github.com/launchdarkly/ld-find-code-refs/blob/master/docs/CONFIGURATION.md#ignoring-files-and-directories) file.

To configure custom delimiters and aliases for your flag keys, or to set other advanced configuration, use the [`.launchdarkly/coderefs.yaml`](https://github.com/launchdarkly/ld-find-code-refs/blob/master/docs/CONFIGURATION.md#advanced-yaml-configuration) file.
