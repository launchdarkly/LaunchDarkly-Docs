...
3. Click **Create a workflow**, and then choose the option to build a **Progressive rollout**. Click **Next**.
...
5. Click **Review workflow** to review the workflow that LaunchDarkly creates based on your configuration.
6. Give your workflow a name, and optionally add a description.
7. (Optional) If you have an approvals process, you can remove the first **Schedule** step, add an approval step, and then re-add the schedule step.
...
2. Click into the **Reviewers** dropdown and choose an approver.
3. Enter a description in the **Description** field.

You must add approval steps before you add schedule steps. If you already have a schedule step, delete the schedule step, and then add the approval step as the first step of that stage.

</Details>

8. Once the workflow looks correct, click **Start workflow**. 
...
1. Navigate to the flag dashboard and select the flag you want to create a workflow for. The flag's Targeting tab appears.
2. Click on the **Workflows** tab.
3. Click **Create a workflow**, and then choose the option to build a **Custom workflow**.
