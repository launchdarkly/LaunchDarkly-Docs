...
A Big Segment is a kind of segment that is backed by an external data store. Big Segments can support massive numbers of targets. The maximum number of targets in a Big Segment is dictated only by the limits of the data store you configure it with, while a standard user segment is capped at 50,000 users. To use Big Segments, you must meet additional prerequisites. To learn more, read [Using Big Segments](#using-big-segments).
