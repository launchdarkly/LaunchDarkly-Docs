...
## Importing into and exporting from Big Segments

You can import users into a Big Segment from a CSV file, as well as export users from a Big Segment into a CSV file.
...
1. Navigate to the desired segment.
2. In the **Included users** search box, enter the key of the user you want to add or remove. If the user is present in
the segment, a **Remove user** button appears. If the user is not present in the segment, an **Add user** button appears.

![Add individual user](big-segment-add-individual-user.png)
