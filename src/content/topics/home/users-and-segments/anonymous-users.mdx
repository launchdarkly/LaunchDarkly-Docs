...
You can designate any user as an anonymous user. Anonymous users work just like regular users, except that they don't appear on your **Users** list in LaunchDarkly. You can't search for anonymous users on your **Users** list, and you can't search or autocomplete by anonymous user keys. Anonymous users still count toward your MAU limit.

The most common reason to designate anonymous users is to prevent unauthenticated users from diluting useful data in the **Users** list.
...
You can use anonymous users to hide personally identifiable information (PII), but we recommend using private user attributes instead. To learn more, read [Using private user attributes](/home/users/attributes#using-private-user-attributes).
...
Using a shared user key for all anonymous users instead of unique keys helps reduce your client-side MAU usage. To learn more about user limits, read [Understanding the Users list](/home/users/users-list#understanding-the-users-list).
...
You can replace some of the unique IDs in the examples above with anonymous users. This decreases your maximum user count, but keep in mind that anonymous users do not appear on the **Users** list. Assess the components interacting with LaunchDarkly and feature flags to determine which require unique IDs, user IDs, or both, or can be classified as anonymous users.

You can remove users you don't need anymore by clicking **Delete user** in the overflow menu for that user on the list, or by opening the user's page and clicking **Delete**. Customers on a Pro or Enterprise plan can also remove users using the LaunchDarkly API. To learn more, read [Removing a user](/home/users/users-list#removing-a-user).
