...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell>This can be a Boolean or string value. This attribute prevents the user from appearing on the dashboard. If the anonymous attribute is set to _any_ value other than boolean `false` or null, LaunchDarkly treats the as user anonymous.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>os</code>
...
If an attribute is marked as private and that user is listed in the `Users` dashboard, we will honor that in the future and continue to mark the attribute as being private even if you decide not to mark it as private in subsequent evaluations. To remedy this, you would need to delete the user from the dashboard and reevaluate the user with the non-private attributes.
