...
By default, all of the attributes in your user objects are sent to LaunchDarkly. This data lets LaunchDarkly determine the expected flag variations for users and powers the autocomplete functionality throughout the LaunchDarkly UI.
...
If you create a custom attribute with a key already in use by a built-in attribute, the SDK will behave unpredictably during feature flag evaluation. For a list of built-in attributes and their keys, read [Understanding user attributes](#understanding-user-attributes).
