...
The **Users** dashboard shows only cached user information. Users appear on the dashboard for 30 days from creation. If a flag sends a `identify` or `flag evaluation` event for a user less than 30 days after it sent the previous event, the user will remain on the dashboard.
...
Accounts are limited to 100,000,000 users. If you reach the users limit, flag evaluations will still work for new users, but they won't appear on the dashboard until stale users age out of the 30-day user cache and your users drop below 100,000,000. To learn more, read [Understanding client-side MAU limits](/home/billing/usage-metrics#understanding-client-side-mau-limits).

If you want to increase your client-side Monthly Active Users (MAU) limit to see more users from your client-side SDKs on the dashboard, contact [Support](https://support.launchdarkly.com/hc/en-us/requests/new).
...
When you schedule a user to be removed from a flag, a **clock icon** appears next to the flag's variation. Hover over it to see when that user will be removed.
...
Customers on a Pro or Enterprise plan can also use the LaunchDarkly API to delete users. To learn more, read the [API docs](https://apidocs.launchdarkly.com/tag/Users#operation/deleteUser).
