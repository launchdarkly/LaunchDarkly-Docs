...
There are two ways to add user targets to a Big Segment:
...
You can import user targets to a Big Segment using a CSV file. After you populate a Big Segment with user targets, you can then add or remove individual users.

### Importing user targets from a CSV file

To import user targets from a CSV file:

1. Navigate to the "Individual targets" section of the Big Segment you wish to import user targets into.

![The "Targeting" tab of an empty Big Segment.](big-segment-empty.png)

2. Click **Upload CSV**. An "Upload CSV" dialog appears.

If the Big Segment already contains user targets, you have the option to merge or overwrite the existing user targets with the user targets from the CSV file.
 - Select "Overwrite" to replace all existing user targets in the segment
 - Select "Merge" to add to the existing user targets in the segment
...
The import process begins in the background. When the import completes successfully, you receive an email notification. Depending on the number of user targets in the CSV, this may take several minutes or hours.
...
After you add user targets to a Big Segment using a CSV file, you can then add or remove individual user targets. 

To add or remove individual user targets to or from a Big Segment:

1. Navigate to the desired segment.
2. In the **Included targets** search box, enter the key of the user target you want to add or remove. If the user target is present in the segment, a **Remove user** button appears. If the user is not present in the segment, an **Add user** button appears.
