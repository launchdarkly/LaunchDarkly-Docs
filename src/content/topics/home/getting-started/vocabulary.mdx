...
A `context kind` is a conceptual grouping of information that you may know about different contexts. Each context includes a `kind` that describes the type of attributes it contains.

To learn more, read [Understanding contexts and context kinds](/home/contexts#understanding-contexts-and-context-kinds).

### Context instance

A `context instance` is a particular individual context that has encountered a feature flag in your product.
...
An `environment` is an organizational unit contained within a [project](#project). You can create multiple environments within each project. All environments in a single project contain the same [flags](#flag). However, the flags can have different states, targets, and rules in each environment.
...
A `project` is an organizational unit for flags in your LaunchDarkly account. You can define projects in any way you like. A common pattern is to create one project in your LaunchDarkly account for each product your company makes.
...
Every member must have at least one role or custom role assigned to them, even if the role explicitly prohibits them from accessing any information within LaunchDarkly.
...
A `user` is a person, service, machine, or other resource that encounters feature flags in your product.

Newer versions of the LaunchDarkly SDKs replace users with [contexts](#context).
