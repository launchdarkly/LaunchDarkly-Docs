...
`MAU` is a billing metric that measures the number of user contexts your flags encounter from client-side SDKs over a 30-day period. MAU includes user contexts that are both single contexts and those that are part of a multi-context. If you are being billed by MAU your MAU appear on the Contexts list, and expire from the list after 30 days of inactivity.

To learn more, read [Understanding MAU usage metrics](/home/billing/usage-metrics#understanding-mau-usage-metrics).

### Monthly context instances (MCI)

`MCI` is a billing metric that measures the number of context instances your flags encounter from client-side SDKs over a 30-day period. If you are being billed by MCI, your MCI appear on the Contexts list and expire from the list after 30 days of inactivity.

To learn more, read [Understanding MCI usage metrics](/home/billing/usage-metrics#understanding-mci-usage-metrics).
