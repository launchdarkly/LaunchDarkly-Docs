...
All of your SDKs must be configured to send events. You may have disabled sending events to reduce your monthly active users (MAU). If so, you must re-enable it. To learn how, read [Sending custom events](/sdk/features/events).
...
11. (Optional) To disable traffic variation reassignment, click **Advanced**, then check the **Keep users assigned to initial variation** checkbox. We strongly recommend leaving this box **unchecked**. To learn more, read [Understanding variation reassignment](#understanding-variation-reassignment).
