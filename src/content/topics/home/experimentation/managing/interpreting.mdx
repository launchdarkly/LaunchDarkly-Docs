...
- Stop the experiment or resume recording. To learn more, read [The experiment lifecycle](/home/experimentation/lifecycle).
- Edit metrics connected to the experiment. Edit experiment metrics to start new experiments using this flag.
- Start a new experiment iteration. This creates a new timeframe for experiment results.
- Delete the experiment data. This leaves the metric connected to the flag, but removes all the data the experiment has collected so far. After the historical data is deleted, the experiment begins collecting new data.
