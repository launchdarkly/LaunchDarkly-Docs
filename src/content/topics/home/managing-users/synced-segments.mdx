...
<Callout intent="info">
  <CalloutTitle>This feature is for Early Access Program cusomers only</CalloutTitle>
  <CalloutDescription>
...
<Callout intent="info">
  <CalloutTitle>Synced segments are not avilable for server-side SDKs or the LaunchDarkly Relay Proxy</CalloutTitle>
  <CalloutDescription>

Synced segments are not supported on server-side SDKs or the LaunchDarkly Relay Proxy.

Support for Server-side SDKs and the LaunchDarkly Relay Proxy is in development, but not yet available as part of the Early Access Program. If a server-side SDK or the Relay Proxy evaluates a synced segment, the segment’s target list appears empty in the LaunchDarkly UI.
