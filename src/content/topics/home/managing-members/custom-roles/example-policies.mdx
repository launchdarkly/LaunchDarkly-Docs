...
Use the `viewProject` action to allow or deny members viewing and editing access to one or more projects. Use the built-in `no access` role to deny viewing and editing access to all projects. To learn more, read [Creating private projects with custom roles](/home/organize/private-projects).
