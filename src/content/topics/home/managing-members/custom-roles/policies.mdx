...
- Any resource or action not specifically included within a policy is denied by default, except the `viewProject` action and the `createAccessToken` action in policies with a `reader` base.
