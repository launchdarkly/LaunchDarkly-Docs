---
path: '/home/advanced/relay-proxy-enterprise/automatic-configuration'
 ...
 * If you configured the Relay Proxy with a configuration file, specify the Relay Proxy key in an `AutoConfig` section as the `key` property and save.
 * If configured the Relay Proxy with environment variables, specify the Relay Proxy key with the `AUTO_CONFIG_KEY` environment variable.
