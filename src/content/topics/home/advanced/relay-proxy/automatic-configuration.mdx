---
path: '/home/advanced/relay-proxy/automatic-configuration'
...
The LaunchDarkly Relay Proxy automatic configuration feature is only available for customers on an Enterprise plan. To enable it on your plan, contact [Sales](mailto:sales@launchdarkly.com).
...
1. Create a LaunchDarkly Relay Proxy configuration in the [Relay Proxy section](https://app.launchdarkly.com/settings/relay) of the Account Settings page and save its unique key.
...
1. Open your Relay Proxy configuration file.
2. Configure your Relay Proxy to use your new key.
 * If configured with a configuration file, specify the Relay Proxy key in an `AutoConfig` section as the `key` property.
 * If configured with environment variables, specify the Relay Proxy key with the `AUTO_CONFIG_KEY` environment variable.
