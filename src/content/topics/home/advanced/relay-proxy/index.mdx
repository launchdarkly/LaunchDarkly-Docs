...
We recommend that all customers on LaunchDarkly's Enterprise plan use the Relay Proxy. Customers on LaunchDarkly's Starter and Professional plans may optionally choose to use the Relay Proxy, however, the associated operating costs with onboarding the Relay Proxy may be prohibitive; see our [best practices](#best-practices-for-using-the-relay-proxy) for more information on operational concerns.

Another deciding factor is based on your usage of LaunchDarkly's [client-side and server-side SDKs](/sdk/concepts/client-side-server-side#understanding-the-different-types-of-sdks). Relay Proxy provides performance and resilience improvements for all server-side SDKs and for SDKs configured to poll. This means that the Relay Proxy works best with the default SDK configurations for all server-side SDKs and for the [client-side JavaScript SDK](/sdk/client-side/javascript). Relay Proxy does not currently scale well when maintaining streaming connections with a large number of client-side SDKs (including mobile SDKs); when utilizing LaunchDarkly's streaming architecture in a heavily-used client-side or mobile application, connecting directly to LaunchDarkly's main service may be most optimal.

All in all, the Relay Proxy was developed to address specific scenarios:
...
If you use a [persistent feature store](/sdk/concepts/feature-store) and you have a large number of servers connected to LaunchDarkly, each server attempts to update the data store when a flag update occurs. This behavior is safe, but inefficient.
...
Note that the Relay Proxy works best with LaunchDarkly's server-side SDKs and with SDKs configured for polling. Relay Proxy does not efficiently handle streaming connections from client-side SDKs. When utilizing LaunchDarkly's streaming architecture in a heavily-used client-side or mobile application, be sure to [monitor](/home/advanced/relay-proxy/monitoring) and scale the Relay Proxy accordingly.
...
If your application is deployed to multiple regions, consider running one or more Relay instances in each of those regions in close proximity to your application. Doing so will limit latency between your application and the Relay Proxy.
