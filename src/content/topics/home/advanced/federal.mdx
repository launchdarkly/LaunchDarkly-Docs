...
Additional language syntax examples for setting configuration options can be found in the documentation for LaunchDarkly relay proxy here (sdk/features/relay-proxy-configuration/proxy-mode), just change the relay proxy URL to the respective federal instance URLs instead. A list of available service endpoints for federal are below.
