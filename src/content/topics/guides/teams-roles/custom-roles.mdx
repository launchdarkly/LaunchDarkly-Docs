...
These policies can now be used as a starting point to create more custom roles, such as members who have write-access to only specific projects.
...
<CodeSample>
<CSTab label="New checkout flow project">
...
<CodeSample>
<CSTab label="View-only by default">
...
[
  ...
  {
    "effect": "allow",
    "actions": ["deleteContextInstances"],
    "resources": ["proj/public:env/*;sandbox,prod:context/*"]
  }
]
```
</CSTab>
...
<CodeSample>
<CSTab label="Allow all actions by default">
...
<CalloutTitle>Using SCIM to provision members</CalloutTitle>
<CalloutDescription>

If you use SCIM to provision members, you will create the roles in LaunchDarkly, but you must configure and manage your account members in the IdP. To learn more, read [Enabling SCIM provisioning](/home/account-security/sso/scim).
...
You can map LaunchDarkly `role` and `customRole` attributes to Azure using Azure claims. `role` and `customRole` can be mapped using any unused Azure source attribute. To learn how, read [Mapping user attributes](/home/account-security/azure#setting-up-custom-roles-for-azure-user-attributes).
...
You can assign custom roles that you created in LaunchDarkly to members through the Okta user interface (UI). To learn how, read [Assigning custom roles in Okta](/home/account-security/okta#assigning-custom-roles-in-okta).

</Details>

<Details summary="Expand OneLogin details">

You can assign custom roles that you created in LaunchDarkly to members through the OneLogin UI. To learn how, read [Adding users and setting roles in OneLogin](/home/account-security/onelogin#adding-users-and-setting-roles-in-onelogin).
