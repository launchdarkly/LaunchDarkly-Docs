...
The Space Camp app uses email addresses as user IDs. Providing the user's email address as the `key` in our user object means that LaunchDarkly stores that email address. We do not recommend this approach for production environments.
...
7. Log in to the Space Camp app as a few different users. Keep track of which email addresses you use as you log in and log out.
