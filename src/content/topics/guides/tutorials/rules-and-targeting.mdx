...
Flags can have multiple rules. If multiple rules exist, LaunchDarkly evaluates them top to bottom based on how the rules are configured in the flag's dashboard. You can drag and drop rules to change the evaluation order.
...
1. Navigate to the [Account settings](https://app.launchdarkly.com/settings/projects) page. It lists all the projects to which your LaunchDarkly account has access.
...
3. Click the flag name. The flag's Targeting page appears. Keep this page open to access it quickly.
...
10. The second field in the form is where you choose the attribute to match. Click and scroll down to find `firstLetter`.
11. Click the third field and choose "is one of". For the targeting rule to succeed, the value of `firstLetter` must match one of the two options you just entered.
12. In the fourth field, enter both the upper and lower case letter `A` as values. Here's how:
    - Click the fourth field to focus it.
...
13. Click the fifth field and choose the `true` variation. Now when the rule matches a user object, the `true` variation appears.
14. In the **Default rule** section, choose `false`. This ensures that only users who match our rules receive a `true` evaluation:
