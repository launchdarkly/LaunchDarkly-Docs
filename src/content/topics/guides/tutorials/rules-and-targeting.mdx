...
* You must have completed the exercises in [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag). The exercises in this guide rely on apps and flags you create in that guide.
* The SDK key for the environment you created in [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag).
...
With LaunchDarkly, you can create user objects with as many attributes as you need. The only required attribute is the `key` attribute, which holds the user identifier. If the LaunchDarkly rules engine sees two user objects with the same `key`, it assumes they represent the same user.
...
In LaunchDarkly, none of the targeting rules need to live in application code. They're all configured using the flag dashboard. Only the user needs to be implemented in your code. This prevents sensitive information from being transmitted with feature flag data.
...
The exercises in this guide rely on the flags and SDK keys you created in the <a href="/guides/tutorials/sdk-flag">Configuring an SDK and creating a flag</a> guide.<br/><br/> If you have not completed that guide, do so now and return to this section.
...
Learn more: [How to use Glitch](https://glitch.com/create)How to use Glitch
...
During the exercises in the [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag) guide, you created a LaunchDarkly project, and configured your app to use the SDK key from that project. This new app will use the same SDK key.
...
1. Find the project which you created for the [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag) guide.
1. Click the project's SDK key to copy it to your clipboard:
...
When you completed the exercises in [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag), you created a flag named Pricing Tier 3.
...
In [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag), you learned about the `variation()` method, which application code uses to evaluate flags.
...
In the example below, the `anonymous` attribute is set to `True`. When this happens, LaunchDarkly won't record any of this user's other attributes or list this user on the Users dashboard.
...
You might see that there are not many users yet. As you continue this guide, you'll add more users to test with.
...
1. Navigate to the **Feature flags** dashboard in LaunchDarkly.
2. In the "Feature flags" list, click Pricing Tier 3. The flag's **Targeting** tab appears.
3. Find the "Default rule", which is the flag variation that appears when targeting is enabled. In this example, it is set to `true`:

![The flag's Default rule selector.](flag-default-rule.png)

2. Click the **Serve** menu.
3. Choose "the "percentage rollout":
...
9. Log out and log back in again as that same user. The third pricing tier appears every time that user logs in, because LaunchDarkly can now identify individual users. That user will always see the third tier until the flag's targeting changes.
10. When you find a user who doesn't see the third tier, save their email address.
...
To target an individual user through the Users dashboard:
...
3. Navigate to the LaunchDarkly **Users** dashboard.
...
8. From the flag dashboard, click the name of the "Pricing Tier 3" flag to return to its **Targeting** tab. The "Target individual users" section is expanded. This section specifies the individual users who should always see a particular flag variation. In the `true` section, you see the key for the user whose setting you just changed:

![The "Target individual users" section of the flag's "Targeting" tab.](flag-targeting-individual-user.png)
...
5. Navigate to the flag dashboard in LaunchDarkly and find the "Pricing Tier 3" flag.
