---
path: '/guides/tutorials/rest-api'
title: 'Getting started with the LaunchDarkly REST API'
...
The LaunchDarkly REST API is a programmatic alternative to interacting with the LaunchDarkly user interface (UI). You can use the LaunchDarkly API to perform any action that's available in the LaunchDarkly dashboard, including creating and manipulating resources such as flags, members, or environments. For example, the REST API is often used for toggling feature flags, updating segments, and building custom integrations.
...
It is not required that you complete any of the other tutorials before reviewing this one. If you have completed the [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag) or [Flag rules and targeting](/guides/tutorials/rules-and-targeting) tutorials, you can use the same projects and flags in the exercises below.
...
![The interactive panel in the API documentation, before any information is entered.](api-documentation-try-it.png)

3. Select the Request tab.
4. In the Security panel, enter your access token under **Authorization**.
5. In the Parameters panel, enter the values for any parameters you are including in the request.

![The interactive panel in the API documentation, with parameters filled in.](api-documentation-interactive-example.png)
...
The LaunchDarkly REST API supports semantic patch for a few of its endpoints. [Semantic patch](#semantic-patch) is described later in this guide. For semantic patch requests, you must append `domain-model=launchdarkly.semanticpatch` to the `Content-Type` header.

### Optional headers

The most commonly used optional header is the `LD-API-Version` header. You can use this to specify a particular API version, including a beta version. If this header is not included, your request uses the latest version of the API.
...
First, you'll read information about a single feature flag. While it's unlikely that you would use the REST API just to read flag information, it's a good introduction to how our APIs work. If you have not yet created a feature flag in your project, follow the procedure in [Your first feature flag](/home/getting-started/feature-flags) or complete the tutorial guide [Configuring an SDK and creating a flag](/guides/tutorials/sdk-flag).
...
If no users have encountered your flag, the `totalCount` is zero. To see what the response looks like with several users, complete the [Flag rules and targeting](/guides/tutorials/rules-and-targeting) tutorial and repeat this request using the account, project, and flag you used in those exercises.
...
* `path`: the reference to the field you want updated, within its JSON object. For example, you can use `"/definition"` for the field named "definition" at the top level of your feature flag object.
...
* **Update feature flags.** You already learned how to [update a feature flag](#updating-a-flag) with a `PATCH` request. You can also build conditional logic into your updates by taking advantage of the additional operations that are available within a JSON Patch request, or explore the operations available through semantic patch. To learn more, read [Updates](https://apidocs.launchdarkly.com/#section/Overview/Updates). Some examples of common reasons to update feature flags include:
  ...
  * Adding or removing user targets, or adding or removing target rules. You can change a flag's targeting programmatically. To learn more, read the semantic patch instructions in [Update feature flag](https://apidocs.launchdarkly.com/tag/Feature-flags#operation/patchFeatureFlag).
...
* **Contact the LaunchDarkly Support team.** If you're considering building out significant functionality with the REST API, [contact Support](https://support.launchdarkly.com/hc/en-us). There may already be a complete or partial solution available that doesn't require custom code on your part.
