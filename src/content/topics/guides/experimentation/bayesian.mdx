...
After we have the posterior distribution for each variant, we use Monte Carlo sampling (not to be confused with Markov Chain Monte Carlo methods, which we do not use) to determine the probability of each variant winning. Some systems compare each variant to the control, but that approach has the same problem as frequentist statistics in determining the winner with multiple variants.
