...
We don't recommend trying to use the SDK aliasing feature to serve the same variation to someone both before and after they log in.
...
- There are some situations when it is not possible for LaunchDarkly to associate two different contexts using the alias method.
...
Choose "ms" as the unit of measure, your success criteria as "Lower than baseline," and `user` as the context kind.
