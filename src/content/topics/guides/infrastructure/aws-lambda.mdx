...
After you download the function locally, install the [LaunchDarkly Node.js (server-side) SDK](/sdk/server-side/node-js).
...
5. Find the "Default rule" section and choose "A percentage rollout" from the "Rollout" menu. For the purposes of this example, assign 50/50. In a real-world scenario, you'd likely start with a smaller distribution in the first variation and increase that number over time.
