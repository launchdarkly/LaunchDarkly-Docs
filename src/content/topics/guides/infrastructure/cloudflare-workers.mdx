...
<Callout intent="info">
<CalloutTitle>A custom Worker build is required in Wrangler 2</CalloutTitle>
<CalloutDescription>

Wrangler 2 supports a new, streamlined Workers integration with Pages, Cloudflare's solution for deploying sites. However, the Cloudflare Pages integration with Workers does not allow for providing a custom Webpack configuration, which is required for LaunchDarkly's Cloudflare Edge SDK. For this reason, you will need to configure your Workers to use a [custom build](https://developers.cloudflare.com/workers/wrangler/configuration/#custom-builds).
...
### Using Webpack to create a custom build

The LaunchDarkly Cloudflare Edge SDK requires that you build the Edge Worker code using a [custom build](https://developers.cloudflare.com/workers/wrangler/configuration/#custom-builds) with [Webpack 4](https://webpack.js.org/). Here's how to set up Webpack 4 and tell wrangler to perform a custom build:

1. Install Webpack 4, the Webpack CLI, and the TypeScript loader for Webpack:

  <CodeSample>
  <CSTab label="npm">

  ```bash
  npm i -D webpack@4 webpack-cli ts-loader@8
  ```
  ...
  ```bash
  yarn add -D webpack@4 webpack-cli ts-loader@8
  ```

  </CSTab>
  </CodeSample>

2. Add a new file to your project named `webpack.config.js` and populate it with the following:

  <CodeSample>
  <CSTab label="webpack.config.js">
...
3. Replace the contents of `tsconfig.json` with the following:

  <CodeSample>
  <CSTab label="tsconfig.json">
...
4. Modify `wrangler.toml` to tell Wrangler that you want to perform a [custom build](https://developers.cloudflare.com/workers/wrangler/configuration/#custom-builds) before your Worker is published:

  <CodeSample>
  <CSTab label="wrangler.toml">

  ```toml
  // Point Wrangler at the output of the transpiled Worker script.
...
5. Use Wrangler to run the site locally again to confirm your Worker builds successfully:
...
4. Open `wrangler.toml` and add the namespace ID to the `kv_namespaces` configuration. If this configuration key does not exist yet, create it. If it does exist with the KV namespace that was created for your site assets, add the new namespace to the array of namespaces.
  ...
  ```toml
  kv_namespaces = [
     {binding = "MY_KV", id="<MY_KV NAMESPACE_ID>"},
  ]
  ```
...
1. Install the SDK:
  ...
  ```bash
  npm i launchdarkly-cloudflare-edge-sdk
  ```
...
5. Open the `/assets/custom.js` file and add the following code at the top of the file, replacing `<LAUNCHDARKLY_CLIENT_ID>` with your LaunchDarkly project's client ID. This tells LaunchDarkly to [bootstrap the client](/guides/flags/static-sites#bootstrapping-the-client) using the injected script.
...
2. Create a function to handle asynchronously retrieving the flag values. You can reuse the LaunchDarkly client you created in the prior example, but you'll get flag values in multiple places, so it's simpler to create a new function.
  ...
  The following function passes in a user `key` and a `user` object and returns the value of the flag for that user. If you do not pass a user, it defaults to `anonymous` as a user key. You can place this function anywhere within the `/src/index.ts` Worker file that is outside an existing code block.
