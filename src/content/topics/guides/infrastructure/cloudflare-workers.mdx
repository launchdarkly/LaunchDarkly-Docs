...
5. Copy and store the namespace ID as you'll need it to enable LaunchDarkly's Cloudflare integration.
  ...
  <ul>
  <li>If you plan to use the [Cloudflare Workers preview service](https://cloudflareworkers.com/), you will need to create a Cloudflare Workers preview namespace as well. To set this up, follow the [Cloudflare integration docs](/integrations/cloudflare#creating-a-cloudflare-worker-with-the-@launchdarkly/cloudflare-server-sdk").</li>
  <li>If you are using the <code>nodejs_compat</code> compatibility flag, you must use <code>wrangler dev --experimental-local</code> to start your local server. To learn more, read [Cloudflare's Node compatibility docs](https://developers.cloudflare.com/workers/wrangler/configuration/#node-compatibility).</li>
  </ul>
