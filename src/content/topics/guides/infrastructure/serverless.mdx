...
In most cases, the best way to use LaunchDarkly in serverless and short-lived environments is with the Relay Proxy hosted on a long-lived server. To learn more about common uses cases for the Relay Proxy, read [Deciding whether to use the Relay Proxy](/home/relay-proxy#deciding-whether-to-use-the-relay-proxy). To learn how to configure the Relay Proxy in your environment, read [The Relay Proxy](/home/relay-proxy).
...
![Creation of a LaunchDarkly webhook which sends flag updates to a Lambda function.](lambda-webhook.png)<br/>
...
## Sending analytics events to the Contexts list

The **Contexts** list is populated with data from analytics events. You must call close in your SDK before shutting down to ensure the SDK sends analytics events and your **Contexts** list displays context data. To learn how, read [Shutting down](/sdk/features/shutdown).
