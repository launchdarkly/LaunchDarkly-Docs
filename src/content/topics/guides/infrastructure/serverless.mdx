...
In many cases, the best way to use LaunchDarkly in serverless and short-lived environments is with the Relay Proxy hosted on a long-lived server. To learn more about common uses cases for the Relay Proxy, read [Relay Proxy use cases](/home/relay-proxy/use-cases). To learn how to configure the Relay Proxy in your environment, read [Implementing the Relay Proxy](/home/relay-proxy/implementing).
