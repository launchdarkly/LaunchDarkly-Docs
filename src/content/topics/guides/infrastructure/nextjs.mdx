...
2. Navigate to **Site Settings**, then **Build & Deploy**, then **Build Hooks**.
...
1. Click on the site you would like to connect the webhook to.
2. Navigate to **Settings**, then **Git**, then **Deploy Hooks**.
...
2. Paste this code into the advanced editor. Change the project key from `proj-key-123abc` to your LaunchDarkly project key. You can get the project key from the **Projects** tab under **Account settings**.
    ...
    [
      {
        "effect": "allow",
        "actions": ["*"],
        "resources": ["proj/proj-key-123abc:env/*:flag/*;build-impact"]
      }
    ]
    ```
...
1. Within the site's settings on the dashboard, go to **Build & Deploy**, then **Environment**.
2. Click **Edit Variables**.
3. Click **Add Variable**.
4. Add the `LAUNCHDARKLY_SDK_KEY` variable.

Alternatively, if you are deploying to Vercel:
1. Open the site in Vercel's dashboard and go to **Settings**, then **Environment Variables**.
...
To get your LaunchDarkly SDK key, go to **Account settings**, then [**Projects**](https://app.launchdarkly.com/settings/projects). Choose your project and then copy the SDK key for the environment you wish to work with.
...
In statically pre-rendered pages, you cannot use different variations for different contexts or roll out a variation. Every context receives the same statically rendered content. This is why you use a single constant value for the context. In this case, you can just use an email as the constant.
...
The client-side SDK uses a different, more restricted key than the server-side SDKs. To get the client-side ID, go to **Account settings**, then [**Projects**](https://app.launchdarkly.com/settings/projects). Copy the ID and add a `LAUNCHDARKLY_SDK_CLIENT_SIDE_ID` value to `.env.local`. You also must verify that this ID exists on your deployment provider.
...
When the context hits this route but `show-about-us` is false, you can make it return a 404, as if the page doesn't even exist.
...
2. Because you introduced a build impact, you cannot show different variations to different contexts. Either everyone will receive the About Us page or no one will.
...
The example below uses a dynamic "catch all" route as `/dpr/[[slug]].js`. The folder name `/dpr` is just for example purposes and not required to use DPR. This page pulls the top five posts from the featured author on the [DEV API](https://developers.forem.com/api) site, as determined by the `featured-username` flag, and prerenders those. This is determined by only adding these five paths to the `paths` variable returned by `getStaticPaths()`.
