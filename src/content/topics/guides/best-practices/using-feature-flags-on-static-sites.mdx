---
path: '/guides/best-practices/using-feature-flags-on-static-sites'
title: 'Using feature flags on static sites'
...
You can use a CMS to manage the creation, management, and publishing workflow of a website. There are many different CMS' like [Contentful](https://www.contentful.com/) and [Prismic](https://prismic.io/).
...
[LaunchDarkly's SDKs](/sdk) can operate on a server or on a client. Static sites must use the [LaunchDarkly client-side JavaScript SDK](https://docs.launchdarkly.com/sdk/client-side/javascript). 
...
To learn more, read [Anonymous users](/sdk/client-side/javascript#anonymous-users).
...
On SPA sites, this may not be a problem. Users will likely only load a site once per browser session. In this case, [subscribe to flag changes](#releasing-features-to-your-clients-in-real-time) to trigger the appropriate behavior in your app in real-time.
...
```js
// If your server passes values to your frontend on pageload,
...
To learn more, read [Bootstrapping the client](/sdk/client-side/javascript#bootstrapping).
...
To learn more, read [Subscribing to feature flag changes](/sdk/client-side/javascript/#subscribing-to-feature-flag-changes).

## Using React

[React](https://reactjs.org/) is a very popular JavaScript UI library. LaunchDarkly has a [React SDK](https://docs.launchdarkly.com/sdk/client-side/react) that lets developers use LaunchDarkly in their React app. One of the helpers the React SDK provides is called [`asyncWithLDProvider`](https://docs.launchdarkly.com/sdk/client-side/react#asyncwithldprovider). It ensures that the flags are available to your React components when they render.
...
This example uses [React hooks](https://reactjs.org/docs/hooks-intro.html). If you are using a version of React without hooks support, you can use the higher-order component [`withLDConsumer`](https://docs.launchdarkly.com/sdk/client-side/react#withldconsumer) instead:
