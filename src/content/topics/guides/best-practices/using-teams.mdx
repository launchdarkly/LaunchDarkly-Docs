...
Alternatively, you can update each of the team member's roles to a role with less access, such as the no access or reader built-in roles, or a custom role with limited access. To learn more, read [Configuring roles with no access](/guides/tutorials/no-access).
