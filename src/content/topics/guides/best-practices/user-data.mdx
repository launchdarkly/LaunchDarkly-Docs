...
- You must make available each flag that you want client-side SDKs to be able to access. To do this, enable the "Make this flag available to client-side SDKs" setting in the flag's configuration. To learn more, read [Client-side ID](/sdk/concepts/client-side-server-side#client-side-id).
...
You can configure a client-side SDK to use the REPORT HTTP verb instead of GET. In this
model, although the user object is sent back to LaunchDarkly as binary data in the request
body, it is not stored anywhere and does not appear in request logs.
...
When an SDK uses REPORT, the SDK operates in polling mode. The SDK connects
...
Using REPORT requires an additional configuration change when the JavaScript SDK initializes.
