---
path: '/guides/best-practices/entitlements'
...
As you read the flag information, you can see that this flag controls a couple of other flags. If you turn `perm-PaidUser` off, it will affect those flags. You can also see that it is a permanent flag, so you know you shouldn’t delete it without extensive discussion. You can see the tags that will help group it with similar flags and make sure that only senior-level operators can change it.
...
Entitlement flags are going to be a part of your code for the foreseeable future, so it’s important to build a plan for testing them and making sure that they remain healthy. We’ve found that it’s easy for long-lived flags to run so well and silently that the default and fallback values may get stale.
...
- [Creating your first flag](https://docs.launchdarkly.com/guides/tutorials/first-flag)
- [Flag rules and targeting](https://docs.launchdarkly.com/guides/tutorials/rules-and-targeting)
- [Feature flag hierarchies](https://docs.launchdarkly.com/guides/best-practices/flag-hierarchy)
