...
2. Once the app has finished initializing, the in-memory flag store is used for flag evaluation functions such as `variation`. These functions run instantly, with no need to wait for network input/output (I/O).
