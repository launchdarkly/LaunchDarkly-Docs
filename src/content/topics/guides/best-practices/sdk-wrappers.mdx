...
You can also encapsulate an `LDClient` instance with a user context so you can pass them as a single object. This is the existing pattern provided by our client-side SDKs. It’s also useful for server-side SDKs in apps that have to evaluate multiple flags for the same user as part of a request or session.

In this use case, the object also acts as a single request/session context, which is useful for observability purposes. To learn more, read [Observability](#observability).
...
For example, in situations where the code needs to be sure that a `false` value is coming from the flag evaluation, and not a fallback served due to lack of connection to or data from LaunchDarkly. Another example is that rules that use Big Segments may also evaluate as `true` even if the Big Segment data is unavailable. The only way to diagnose this problem is by checking the `EvaluationReason`.
...
When you combine the SDK and API in a wrapper, the wrapper uses the LaunchDarkly REST API to fetch metadata about each flag, such as its title, description, tags, and custom attributes. You can use the metadata to describe policies, fallbacks, and other information the wrapper needs. This gives you more power over flag and SDK behavior without needing additional user interface (UI) or services.
