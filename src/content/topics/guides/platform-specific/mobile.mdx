...
We recommend using `LdClient.InitAsync(key, user)` to initialize the SDK, as it runs asynchronously. The alternative method, `LdClient.Init(key, user, timeout)` blocks until either it has retrieved flag data _or_ it has exceeded the timeout period.
...
Depending on how `init()` is called, the SDK may block the current thread until _either_ the flags are retrieved _or_ the timeout period is exceeded. If `init()` is called in the main app thread, the rest of the app startup will be blocked until this call completes, resulting in poor user experience.
