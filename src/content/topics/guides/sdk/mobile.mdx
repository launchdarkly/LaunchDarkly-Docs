...
We recommend using `LdClient.InitAsync(key, context)` to initialize the SDK, as it runs asynchronously.
...
For each flag used by your app, decide on a policy for how updates to the flag should be handled while the app is running. You may want to ignore updates to certain flags if they happen in the middle of a customer session, whereas other flags, such as kill switches, should update immediately.
...
The SDK communicates evaluation and diagnostic data back to the server as part of normal usage. The LaunchDarkly UI displays this data in several places, including the [flags list](/home/organize/flags-list), the [flag Insights tab](/home/flags/insights), and in [Experimentation](/home/about-experimentation).
...
We recommend creating a "device" context kind to track information about each end user's operating system and hardware, such as operating system and device, so that it's easier to target specific configurations. To learn more, read [Context kinds](/home/contexts/context-kinds). Additionally, we recommend that your code set the application version, which you can do as part of your SDK [Configuration](/sdk/features/config).

This combination of attributes is particularly useful for targeting the release of new features. For example, consider a “dark mode” feature being added to an app. Versions 10 to 14 contain early, incomplete versions of the feature. These versions are available to all customers, but the “dark mode” feature is only enabled for testers. With version 15, the feature is considered complete and enabled for all customers. Unfortunately, some customers are still using an older version of the app. When the feature is enabled, they encounter bugs.
