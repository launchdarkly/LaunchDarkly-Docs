...
description: 'This topic explains the API changes made in theC/C++client SDK 2 release and how to migrate to that version.'
published: true
---
...
For more advanced `cmake` usage, or if you're using another platform, read this [cmake guide](https://preshing.com/20170511/how-to-build-a-cmake-based-project/). The SDK is exported as the `ldclientapi` target.

The repository organization of source files should not be considered stable. If you wish to build your own artifacts, and use them outside of a cmake project you should `cmake install` to your directory of choice which will copy the required headers and binary.
...
In version 1.x of the SDK, when you compiled it with C++, `LDClient` would be defined as a class instead of a C struct. We removed this system in version 2.0, and now `LDClient` is always a C style struct. 
