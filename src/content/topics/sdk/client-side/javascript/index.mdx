...
Code samples on this page are from the two most recent SDK versions where they differ. To learn more about upgrading, read [JavaScript SDK 2.x to 3.0 migration guide](/sdk/client-side/javascript/migration-2-to-3).
...
Additionally, the JavaScript SDK versions 3 and higher use optional chaining. You can ensure support for optional chaining by using Webpack 5. If you are using Webpack 4, you must manually update your `package.json` file. To learn more, read [Dependencies for the v3 SDK](/sdk/client-side/javascript/migration-2-to-3#dependencies-for-the-v3-sdk).
...
After you install the dependency, initialize the LaunchDarkly client. To create a client instance, you need your LaunchDarkly environment's client-side ID.
...
<CodeTabs
  defaultValue="js"
  values={[
    { label: 'JavaScript SDK v3 (JavaScript)', value: 'js', },
    { label: 'JavaScript SDK v3 (TypeScript)', value: 'ts'},
    { label: 'JavaScript SDK v2.x (JavaScript)', value: 'js-v2', },
    { label: 'JavaScript SDK v2.x (TypeScript)', value: 'ts-v2'},
  ]
}>
<CodeTabItem value="js">
...
You must make feature flags available to client-side SDKs before the SDK can evaluate those flags. If an SDK tries to evaluate a feature flag that is not available, the end user will receive the default value for that flag.
