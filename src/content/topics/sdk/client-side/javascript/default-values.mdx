...
When using LaunchDarkly on your website the content the user receives will often depend on the values of your feature flags. If the page is initially rendered with one set of flag variations then updated with a new set it can cause a jarring switch between two views of the content. This can appear as a "flash" or "flicker" of the initial content during a page load. Usually we don't expect this to happen, but under certain use cases extra steps might be necessary to prevent it.
...
If you are using the React SDK, another option is to use the `asyncWithLDProvider` function. Read the [React Web SDK reference](/sdk/client-side/react/react-web#initializing-using-asyncwithldprovider) for more information.
