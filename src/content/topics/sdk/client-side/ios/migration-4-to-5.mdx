...
The minimum platform targets have changed.
...
In the 5.0 release of the iOS SDK, the `LDClient.shared` property was removed, and `start` is now a static method of the `LDClient` class. To initialize the shared instance(s), `start` is called with the desired SDK configuration and optionally a `LDUser`. After `start` returns, it is safe to retrieve the primary instance with the new `LDClient.get()` static method, or a secondary environment with the `LDClient.getForMobileKey()` static method. See the following code samples for an example.
...
The following were removed from the publically exposed surface of the SDK. These were implementation details that were not intended to be public.
...
* The SDK now has support for multiple LDClient instances that are tied to separate mobile keys, allowing the evaluation of flags in multiple environments. See the [multiple environments](/sdk/client-side/ios#multiple-environments) section of the reference guide for more information on this feature.
...
* The LaunchDarkly iOS SDK is now compatible with the Swift Package Manager. Read more about building with the Swift Package Manager in the [getting started](https://docs.launchdarkly.com/sdk/client-side/ios#getting-started) section of the iOS reference guide.
