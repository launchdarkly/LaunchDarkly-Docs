...
title: 'Client-side .NET SDK 1.x to 2.0 migration guide'
...
<Callout intent="info">
  <CalloutTitle>This migration guide is for a prerelease version of the client-side .NET SDK</CalloutTitle>
  <CalloutDescription>

Client-side .NET SDK 2.0 is not yet generally available. This topic describes the latest `2.0.0-rc.*` prerelease version. This is a living document that may change if we make additional modifications to the SDK before the final release.

The API documentation for 2.0 will not be in GitHub Pages until the GA release, but documentation for any version is available in HTML form by downloading the "Documentation" archive from the [GitHub release](https://github.com/launchdarkly/dotnet-server-sdk/releases) for that version.
...
In version 2.0, there is a new syntax for configuring the SDK to connect to a Relay Proxy instance.
...
```csharp
// 1.x model: proxy mode with streaming
...
// 1.x model: proxy mode with polling
var relayUri = new Uri("http://my-relay-host:8000");
var config = Configuration.Builder("my-mobile-key")
    .IsStreamingEnabled(false)
    .BaseUri(relayUri)
...
```csharp
// 2.0 model: proxy mode with streaming
var relayUri = new Uri("http://my-relay-host:8000");
var config = Configuration.Builder("my-mobile-key")
...
// 2.0 model: proxy mode with polling
var relayUri = new Uri("http://my-relay-host:8000");
var config = Configuration.Builder("my-mobile-key")
...
* The SDK now supports aliasing users. See `LdClient.Alias` and `ConfigurationBuilder.AutoAliasingOptOut`.
* `LdClient.DataSourceStatusProvider` allows you to monitor the status of the connection to LaunchDarkly.
* The `Integrations.TestData` class provides a way to inject simulated feature flag data into the SDK for testing.
