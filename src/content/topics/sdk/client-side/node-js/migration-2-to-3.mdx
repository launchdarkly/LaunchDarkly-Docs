...
<ul>
<li>The fields in your user object must be <code>LDUser</code> fields, not <code>LDContext</code> fields. For example, to mark an attribute as private, you must use <code>privateAttributeNames</code> in the user object, not <code>_meta.privateAttributes</code> as you would for a context object. To learn more, read [Understanding changes to private attributes](#understanding-changes-to-private-attributes).</li>
<li>Any additional attributes in your user object need to be inside the <code>custom</code> property of the <code>LDUser</code>, not at the top-level as they would in a context object. To learn more, read [Working with built-in and custom attributes](#working-with-built-in-and-custom-attributes).</li>
</ul>
