...
Note that the flush interval is configurable-- if you need to change the interval, you can do so in the configuration.
...
2. When the app is backgrounded, the stream connection is terminated and the SDK will poll (with caching) for flag updates every hour (configurable).
4. When the app is foregrounded, we reconnect to the stream which sends the latest flag values.
