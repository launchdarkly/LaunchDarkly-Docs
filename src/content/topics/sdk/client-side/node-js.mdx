...
This SDK is intended for use in *single-user* desktop or Node-enabled device applications. If you have a Node.js application and want to set up LaunchDarkly on the server-side, read the [Node.js SDK reference](/sdk/server-side/node-js). If you are using Electron, there is an [Electron SDK](/sdk/client-side/electron) more specifically designed for that environment.
