...
Because Electron is based on Node.js, it is possible to run the [LaunchDarkly Node SDK](/sdk/client-side/node-js) in it. However, we strongly discourage this because the Node.js SDK is meant for server-side use, not for applications that are distributed to users. There are several reasons why this distinction matters:
