...
It accepts an `AsyncProviderConfig` object which is a subset of the `ProviderConfig` object.
...
The `withLDProvider` function initializes the React SDK and wraps your root component in a `Context.Provider`. It accepts
a [`ProviderConfig`](#configuring-the-react-sdk) object used to configure the React SDK:
...
The [`AsyncProviderConfig`](https://launchdarkly.github.io/react-client-sdk/interfaces/asyncproviderconfig.html) object
provides configuration to the `asyncWithLDProvider` function and the [`ProviderConfig`](https://launchdarkly.github.io/react-client-sdk/interfaces/providerconfig.html) object provides configuration to the `withLDProvider` function.


The only mandatory property is the client-side ID. All other properties are optional. These are the properties:
