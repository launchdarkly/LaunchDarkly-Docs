...
The `config` object can include a variety of options. `mobileKey` is a required property, but all other properties are optional. To learn more about the specific configuration options that are available in this SDK, read the [SDK's API docs](https://launchdarkly.github.io/react-native-client-sdk/index.html#ldconfig).
...
In addition to built-in attributes like names and email addresses, you can pass us any of your own user data by passing custom attributes, like the groups attribute in the example above. Custom attributes are one of the most powerful features of LaunchDarkly. They let you target users according to any data that you want to send to us, including organizations, groups, andaccount plans. Anything you pass to us becomes available instantly on your dashboard.
...
<Callout intent="info">
 ...
 When `LDClient` initializes for the first time at app launch, users receive feature flag default values until a connection to LaunchDarkly completes for the first time. Read the section above for various ways to initialize the client.
...
LaunchDarkly's React Native SDK supports having multiple internal `LDClient` instances tied to separate mobile keys. This allows evaluating flags from multiple environments.
...
To learn more about the return types, read the API documentation about [`getConnectionMode`](https://launchdarkly.github.io/react-native-client-sdk/classes/_launchdarkly_react_native_client_sdk_.ldclient.html#getconnectionmode) and [`getLastFailure`](https://launchdarkly.github.io/react-native-client-sdk/classes/_launchdarkly_react_native_client_sdk_.ldclient.html#getlastfailure).
