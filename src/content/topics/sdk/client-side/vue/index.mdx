...
To install the Vue SDK, you need your LaunchDarkly environment's client-side ID. Your client-side ID is available in the **Projects** tab of your [**Account settings**](https://app.launchdarkly.com/settings/projects) page. Client-side IDs are not secret and you can expose them in your client-side code without risk. Never embed a server-side SDK key into a client-side application.
...
The SDK provides a Vue plugin that you can add to your app. You can pass configuration options to the plugin when it loads, or to the `ldInit` function if you are using the `deferInitialization` option. To learn more, read [Initializing the client](#initializing-the-client). 
<!--- To learn more about the available configuration options, read [LDPluginOptions](https://launchdarkly.github.io/vue-client-sdk/types/LDPluginOptions.html). -->
...
const app = createApp(App)
app.use(LDPlugin, { clientSideID: 'YOUR_CLIENT_SIDE_ID', deferInitialization: true })
app.mount('#app')
```
...
const [ldReady] = ldInit({ user: { key: 'example-user-key', name: 'Sandy' } })
</script>
...
<!--- To learn more, read [the API documentation for ldInit](https://launchdarkly.github.io/vue-client-sdk/functions/ldInit.html). -->
...
<!--- To learn more, read [the API documentation for useLDFlag](https://launchdarkly.github.io/vue-client-sdk/functions/useLDFlag.html). -->

You may also access the LaunchDarkly client with `useLDClient`, the [LDClient](https://launchdarkly.github.io/js-client-sdk/interfaces/_launchdarkly_js_client_sdk_.ldclient.html) object from the underlying JavaScript SDK.
