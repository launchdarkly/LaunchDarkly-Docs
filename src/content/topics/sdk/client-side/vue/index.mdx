...
<Callout intent="info">
<CalloutTitle>Version 2 of the Vue SDK replaces users with contexts</CalloutTitle>
<CalloutDescription>
...
Code samples on this page are from the two most recent SDK versions where they differ. To learn more about upgrading, read [Vue SDK 1.x to 2.0 migration guide](/sdk/client-side/vue/migration-1-to-2).
...
<Table>
  ...
  <TableBody>
    <TableRow>
      ...
      <TableCell>[SDK API docs](https://launchdarkly.github.io/vue-client-sdk/)</TableCell>
    </TableRow>
    <TableRow>
    ...
    <TableRow>
      ...
      <TableCell>[Vue](/sdk/client-side/vue#example-app)</TableCell>
    </TableRow>
    <TableRow>
...
<Callout intent="info">
<CalloutTitle>The Vue SDK uses a client-side ID</CalloutTitle>
<CalloutDescription>

The Vue SDK uses a client-side ID. Your environment's client-side ID is available in the **Projects** tab of your [**Account settings**](https://app.launchdarkly.com/settings#/projects) page. To learn more about key types, read [Keys](/sdk/concepts/client-side-server-side#keys).
...
<CSTab label="App.vue, Vue SDK v2.0">

```html
<script setup>
import { ldInit } from 'launchdarkly-vue-client-sdk'

const [ldReady] = ldInit({ context: { kind: 'user', key: 'context-key-123abc', name: 'Sandy' } })
</script>
...
<CSTab label="App.vue, Vue SDK v1.x">
