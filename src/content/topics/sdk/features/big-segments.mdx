...
Big Segments are a type of segment that can contain an arbitrarily large number of user contexts or users. Big Segments only support contexts with a context kind of "user." To learn more, read [Big Segments](/home/contexts/big-segments).
...
<Table>
  ...
  <TableBody>
    <TableRow>
      ...
      <TableCell>
        The maximum number of users or user contexts whose Big Segments status the SDK can cache. The Big Segments status includes all of the Big Segments the user or user context is a part of. A higher value means that the SDK queries the database for recently-referenced users' or user contexts' Big Segments statuses less often. If not specified, the default value is 1000.
      </TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
