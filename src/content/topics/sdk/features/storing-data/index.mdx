...
To use a persistent feature store without connecting to LaunchDarkly, use `PersistentDataStore()` as above, and then use [`ExternalUpdatesOnly()`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldcomponents#ExternalUpdatesOnly) to configure daemon mode. To learn more, read [Using daemon mode](/sdk/features/relay-proxy-configuration/daemon-mode#go).
</Details>
...
To use a persistent feature store without connecting to LaunchDarkly, use `persistentDataStore` as above, and then use `externalUpdatesOnly` to configure daemon mode.
