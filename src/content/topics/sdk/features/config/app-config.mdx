...
- Application version (string): A unique identifier representing the version of the application where the LaunchDarkly SDK is running. The value can use only alphanumeric characters and hyphens `-`, periods `.`, and underscores `_`. For example, you could use a standard version string, such as `1.0.0`, or a secure hash algorithm (SHA), such as `abc123def456`.
