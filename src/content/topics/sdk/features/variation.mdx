---
path: '/sdk/features/variation'
title: 'Variation configuration'
description: 'This topic explains how to use the variation feature to serve different feature flag variations to users.'
published: true
---

## Overview

This topic explains how to use the `variation` feature to serve different feature flag variations to users.

This feature is available for both client-side and server-side SDKs.
...
The `variation` call automatically creates a user in LaunchDarkly if a user with that user key doesn't exist already. You do not need to create users manually, but if you want to, you can use `identify`. To learn more, read [Identify](/sdk/features/identify).
...
The `Variation` methods determine whether a flag is enabled or not for a specific user. In .NET, there is  a `Variation` method for each type, such as [`BoolVariation`](http://launchdarkly.github.io/dotnet-server-sdk/html/M_LaunchDarkly_Client_LdClient_BoolVariation.htm) and [`StringVariation`](http://launchdarkly.github.io/dotnet-server-sdk/html/M_LaunchDarkly_Client_LdClient_StringVariation.htm).

`Variation` calls take the feature flag key, a [`User`](http://launchdarkly.github.io/dotnet-server-sdk/html/T_LaunchDarkly_Client_User.htm), and a default value. The `Variation` call automatically creates a user in LaunchDarkly if a user with that user key doesn't exist already.
