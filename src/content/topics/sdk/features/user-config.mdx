...
Built-in attributes include common identifiers for users, like `firstName` and `email`. For a list of all built-in user attributes LaunchDarkly supports, read [Understanding user attributes](/home/users/attributes#understanding-user-attributes).

Custom attributes let you target users according to any data that you want to send to LaunchDarkly, including organizations, groups, and account plans. Custom attributes values can be booleans, numbers, strings, or arrays. If you set the value of a custom attribute to a JSON object, it won't appear on your **Users** dashboard. To learn more, read [Setting custom user attributes](/home/users/attributes#setting-custom-user-attributes).
