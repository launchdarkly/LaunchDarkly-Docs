...
Most of LaunchDarkly's built-in attributes, like names and email addresses, expect string values. Custom attributes values can be strings, booleans like true or false, numbers, or lists of strings, booleans, or numbers. If you enter a custom value on the LaunchDarkly dashboard that looks like a number or a boolean, the value is interpreted as a number or a boolean. The Android SDK is strongly-typed, so be aware of this distinction.
