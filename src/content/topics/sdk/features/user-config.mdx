...
auto context = ContextBuilder()
  ...
  .Set("firstName", "Sandy")
  .Set("lastName", "Smith")
  .Set("groups", Value::Object{{"string", "Google"}, {"string", "Microsoft"}})
  .Build();
```
...
To learn how to configure private attributes in the C++ (client-side) SDK, read [Private attributes](/sdk/features/private-attributes#c-client-side).

To learn how to configure anonymous users in the C++ (client-side) SDK, read [Anonymous contexts and users](/sdk/features/anonymous#c-client-side).
</Details>
...
In the Flutter SDK, use a [builder pattern](https://en.wikipedia.org/wiki/Builder_pattern) to construct contexts.
...
To learn how to configure anonymous contexts in the Roku SDK, read [Anonymous contexts and users](/sdk/features/anonymous#roku).
</Details>
...
```ts
const context: ld.LDMultiKindContext = {
  kind: 'multi',
  user: { key: 'user-key-123abc' },
  device: { key: 'device-key-123abc' }
}
...
To configure contexts, the Akamai SDK uses the same code as the [Node.js server-side SDK](#nodejs-server-side).

The Akamai SDK does not support sending events, so [private attributes](/sdk/features/private-attributes) are not supported.
...
To configure contexts, the Cloudflare SDK uses the same code as the [Node.js server-side SDK](#nodejs-server-side).

The Cloudflare SDK does not support sending events, so [private attributes](/sdk/features/private-attributes) are not supported.
...
To configure contexts, the Vercel SDK uses the same code as the [Node.js server-side SDK](#nodejs-server-side).

The Vercel SDK does not support sending events, so [private attributes](/sdk/features/private-attributes) are not supported.
