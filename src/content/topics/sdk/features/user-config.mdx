...
The Cloudflare SDK does not support sending events so private attributes are not supported. To configure contexts, the Cloudflare SDK uses the same code as the [Node.js server-side SDK](#nodejs-server-side).
