...
The track feature lets you record actions your users take in your application as events. You can connect these events to metrics in A/B tests or experiments in the flag dashboard.
...
You can completely disable event sending by setting `sendEvents` to `false` in the client options, but be aware that this means not all user data will appear on the flag dashboard.
...
You can completely disable event sending by setting `sendEvents` to `false` in the client options, but this means not all data will appear on the flag dashboard.
