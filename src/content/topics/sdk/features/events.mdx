...
The track feature lets you record actions your users take in your application as events. You can connect these events to metrics in experiments in the flags list. To learn more about the events SDKs send to LaunchDarkly, read [Analytics events](/sdk/concepts/events).
...
To learn more about click and page view goals, read [About Experimentation](/home/about-experimentation).
...
If you [define click or page view metrics](/home/about-experimentation) in LaunchDarkly, they are sent automatically once the client has been initialized. You do not have to do anything else with the client to send click or page view goals. The SDK will generate `page view` events correctly regardless of how the URL is changed, such as by the HTML5 history API, by changing the URL hash fragment, etc.
