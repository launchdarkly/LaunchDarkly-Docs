...
The track feature lets you record actions your customers take in your application as events. You can connect these events to metrics for use in experiments. To learn more, read [Creating metrics](/home/creating-experiments/metrics).
...
You can record `custom` events using the `track` function. Call `track` with the event name to record custom conversion metrics.
