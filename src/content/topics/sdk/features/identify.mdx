...
Client-side SDKs are configured to operate for one end user at a time, identified or anonymous. In these SDKs, the identify feature allows you to change the context, such as when an end user logs in or changes their settings. Identifying contexts and users causes LaunchDarkly to index them. To associate two contexts with each other, such as an end user before and after they log in, call identify with a multi-context that contains both contexts. The association doesn't persist between calls, so you must call identify with the multi-context each time.
...
In some situations, the new context may be similar to the previous context. For example, on a sign-in page in a single-page app, you could represent the same person as an anonymous user before they log in, and a different user after they log in. After the person logs in, you can call identify so that the person receives the correct feature flag settings for their account.

In other situations, the new context may be a multi-context, that is, a set of several different contexts that you want to evaluate together. For example, as soon as an end user visits your app, you may initialize the client with a context using a context kind of "device." When the end user logs in, you now also have their user and organization information.
...
```c
LDClientSDK_Identify(client, updated_context, LD_NONBLOCKING, NULL);
```
...
Client-side SDKs are configured to operate for one context at a time, whether identified or anonymous. The `identifyWithContext()` method tells the client to change the current context and obtain the feature flag values for the new context.

In some situations, the new context may be an updated version of the existing context. For example, on a sign-in page in a single-page app, you could represent the same person as an anonymous user before they log in, and a different user after they log in. You can initialize the client with an anonymous context with a context kind of "user." After the person logs in, you can update the user context and call `identifyWithContext()` so that the person receives the correct feature flag settings for their account.

In other situations, the new context may be a multi-context, that is, a set of several different contexts that you want to evaluate together. For example, as soon as an end user visits your app, you may initialize the client with a context using a context kind of "device." When the end user logs in, you now also have their user and organization information. You can call `identifyWithContext()` with a multi-context that contains the "device," "user," and "organization" contexts for the end user.

Here's how:

<CodeSample>
<CSTab label="Flutter SDK v2.0 (Dart), single context">
...
<CSTab label="Flutter SDK v2.0 (Dart), multi-context">
...
To learn more, read [`identifyWithContext`](https://pub.dev/documentation/launchdarkly_flutter_client_sdk/latest/launchdarkly_flutter_client_sdk/LDClient/identifyWithContext.html).
...
In some situations, the new context may be an updated version of the existing context. For example, on a sign-in page in a single-page app, you could represent the same person as an anonymous context before they log in, and a different context after they log in. After the person logs in, you can call `identify()` so that the person receives the correct feature flag settings for their account.
...
In server-side SDKs, `LDClientIdentify` creates or updates users in LaunchDarkly, which makes them available for targeting and autocomplete on the **Users** list.

In most cases, you do not need to call `LDClientIdentify`. The `variation` call automatically creates users on the **Users** list for you, using the user object you pass to each `variation` method. `LDClientIdentify` is useful if you want to pre-populate your **Users** list before you launch any features.
...
In most cases, you do not need to call `identify`. The `variation` methods automatically create users on the **Users** list for you, using the user object you pass to each `variation` method. `identify` is useful if you want to pre-populate your **Users** list before you launch any features.
