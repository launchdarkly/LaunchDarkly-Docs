...
title: 'Flushing SDK events'
description: 'This topic explains how to use the flush feature. Flush is available on both server-side and client-side SDKs.'
published: true
---

## Overview

This topic explains how to use the flush feature. `Flush` is available on both server-side and client-side SDKs.

## Using flush

Flush tells the client to send all of an SDK's pending analytics events to LaunchDarkly as soon as possible. Flushing is asynchronous, so this method returns before it is complete.
