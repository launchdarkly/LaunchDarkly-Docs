...
The .NET SDK uses the `Common.Logging` framework. For an example configuration, read the [Common.Logging README](https://github.com/net-commons/common-logging#2-register-and-configure-commonlogging).
...
By default, the SDK uses the [`winston`](https://www.npmjs.com/package/winston) package. All loggers are namespaced under `[LaunchDarkly]`.
...
Winston's syntax for instantiating and configuring loggers changed between versions 2.x and 3.x. If the transitive dependency generates errors, declare a dependency on a specific Winston version.
