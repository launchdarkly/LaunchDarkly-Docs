...
By default, the Java SDK uses [SLF4J](https://www.slf4j.org/). SLF4J has its own configuration mechanisms for determining where output will go, and for filtering by level and/or logger name. It will not generate any output unless you have provided a configuration. For an example of using SLF4J with a simple console logging configuration, visit an SLF4J-specific version of [hello-java](https://github.com/launchdarkly/hello-java/tree/slf4j-logging).

Starting with version 5.10.0, you can also configure logging without using SLF4J. The [`LDConfig.Builder.logging`](https://javadoc.io/static/com.launchdarkly/launchdarkly-java-server-sdk/5.9.0/com/launchdarkly/sdk/server/LDConfig.Builder.html#logging(com.launchdarkly.sdk.server.interfaces.LoggingConfigurationFactory)) method and the [`com.launchdarkly.logging`](https://github.com/launchdarkly/java-logging/) API allow you to change the output destination and log level.
...
<CodeTabs
  defaultValue="java"
  values={[
    { label: 'Java (SDK versions 5.10.x and later)', value: 'java', },
  ]
}>
<CodeTabItem value="java">
