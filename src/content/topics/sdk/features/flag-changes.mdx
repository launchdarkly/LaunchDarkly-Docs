...
You can configure these settings if needed. To learn more, read [Client configuration](/sdk/features/config#android)
...
This configuration means that the device gets near real-time updates for feature flag values when the app is in the foreground. You can configure these settings if needed. To learn more, read [Client configuration](/sdk/features/config#xamarin)
...
* [Go](#go)
* [Java](#java)
* [Node.js (server-side)](#node-server-side)
...
<Callout intent="info">
  ...
  <CalloutDescription>
    The Go SDK only supports Subscribing to flag changes in versions 5.0.0 and higher. Most other server-side LaunchDarkly SDKs do not support this feature. All of the client-side SDKs do support this feature.
  </CalloutDescription>
</Callout>
