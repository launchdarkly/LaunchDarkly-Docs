...
LaunchDarkly manages all flags for a user context by updating the flag cache based on a real-time event stream. When a flag is modified from the flag dashboard, the flag values for the current user update almost immediately.
...
The default SDK configuration has the best combination of low latency updates and minimal battery drain.

Here's how the default configuration works:

1. When the app is foregrounded, it makes a [Server-sent events](https://en.wikipedia.org/wiki/Server-sent_events) streaming connection to LaunchDarkly. This streaming connection stays open as long as your app is in the foreground and is connected to the internet.
2. When the app is backgrounded, it terminates the stream connection and the SDK polls with caching for flag updates every three minutes.
3. When the app is foregrounded, the SDK reconnects to the stream and fetches the latest flags.

In either the foreground or background, the SDK doesn't try to update unless the device has internet connectivity.
...
When streaming, the SDK actively monitors network availability. It avoids requests when the network is unavailable, and reconnects when the network becomes available again. When polling, the SDK checks for network connectivity at the current polling interval, only making the request if the check succeeds. When it reconnects, it automatically syncs its local cache with LaunchDarkly.
...
To accomplish real-time updates, LaunchDarkly broadcasts an event stream that is listened to by the C/C++ SDK. Whenever an event is performed on the dashboard, the C/C++ SDK is notified of the updated flag settings in real-time.
...
LaunchDarkly manages all flags for a user context in real-time by updating flags based on a real-time event stream. When you modify a flag from the flag dashboard, the flag values for the current user update almost immediately.

To accomplish real-time updates, LaunchDarkly broadcasts an event stream that the SDK listens to. LaunchDarkly notifies the SDK of updated flag settings in real-time whenever an event registers on the dashboard.  

Streaming mode is not available on watchOS. On iOS and tvOS, the client app must be running in the foreground to connect to the event stream. On macOS, the client app may run in either the foreground or background to connect to the event stream. If streaming mode is not available, the SDK reverts to polling mode.
...
LaunchDarkly manages all flags for a user context by updating flags based on a real-time event stream. When a flag is modified from the flag dashboard, the flag values for the current user update almost immediately.
...
To listen for changes in flag values for a specific flag key and user, use `getFlagTracker().addFlagValueChangeListener`, which provides `FlagValueChangeEvent`s. This is equivalent to re-evaluating the flag for that user whenever `addFlagChangeListener()` reports a change in that flag. Because flag values can have different data types, the value is reported using the general type `LDValue`.
