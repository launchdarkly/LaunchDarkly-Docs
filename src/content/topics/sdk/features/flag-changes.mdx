...
LaunchDarkly manages all flags for a user context by updating the flag cache based on a real-time event stream. When a flag is modified from the flags list, the flag values for the current user update almost immediately.
...
To accomplish real-time updates, LaunchDarkly broadcasts an event stream that is listened to by the C/C++ SDK. Whenever an event is performed on the flags list, the C/C++ SDK is notified of the updated flag settings in real-time.
...
LaunchDarkly manages all flags for a user context in real-time by updating flags based on a real-time event stream. When you modify a flag from the flags list, the flag values for the current user update almost immediately.

To accomplish real-time updates, LaunchDarkly broadcasts an event stream that the SDK listens to. LaunchDarkly notifies the SDK of updated flag settings in real-time whenever an event registers on the flags list.
...
LaunchDarkly manages all flags for a user context by updating flags based on a real-time event stream. When a flag is modified from the flags list, the flag values for the current user update almost immediately.
