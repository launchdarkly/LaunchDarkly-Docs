...
<CodeTabs
  ...
  values={[
    { label: 'client-side .NET', value: 'dotnet-client', },
    { label: 'client-side .NET SDK versions 1.x', value: 'dotnet-client-pre-2.0', },
  ]
}>
<CodeTabItem value="dotnet-client">
...
LaunchDarkly manages all flags for a user context in real-time by updating flags based on a real-time event stream. When a flag is modified from the flag dashboard, the flag values for the current user updates almost immediately.
...
For both of these event kinds, an extra parameter is sent to event listeners. This object has the single property `key`, with its value set to the flag key. If you listened for the general `"update"` event, this lets you see which flag changed.
