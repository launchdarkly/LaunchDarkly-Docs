...
The `detail.Reason()` response is described in [Evaluation reasons](/sdk/concepts/evaluation-reasons).
...
if (detail.Value()) {
   ...
   if (auto reason = detail.Reason()) {
      // reason might not be present, so we have to check
      std::cout << "Value was false because of " << reason << std::endl;
   } else {
...
```c
LDEvalDetail detail;
if (LDClientSDK_BoolVariationDetail(client, "flag-key-123abc", false, &detail)) {
...
LDEvaluationDetail<bool> detail =
  await LDClient.boolVariationDetail("flag-key-123abc", false);
  // or stringVariationDetail for a string-valued flag, and so on.
...
LDBoolEvaluationDetail *detail = [[LDClient get] boolVariationDetailForKey:@"flag-key-123abc" defaultValue:NO];
...
```go
value, detail, err := client.BoolVariationDetail("flag-key-123abc", context, false)
// or StringVariationDetail for a string-valued flag, etc.
...
```go
value, detail, err := client.BoolVariationDetail("flag-key-123abc", myUser, false)
// or StringVariationDetail for a string-valued flag, and so on.
...
To learn more, read [`LDEvaluationDetail`](https://launchdarkly.github.io/js-core/packages/sdk/server-node/docs/interfaces/LDEvaluationDetail.html) and [`variationDetail`](https://launchdarkly.github.io/js-core/packages/sdk/server-node/docs/interfaces/LDClient.html#variationDetail).
...
To learn more, read [`LDEvaluationReason`](https://launchdarkly.github.io/js-core/packages/sdk/server-node/docs/interfaces/LDEvaluationReason.html).
...
$detail = $client->variationDetail("flag-key-123abc", $myUser, false);
...
```rust
let detail = client.bool_variation_detail(&user, "flag-key-123abc", false);
