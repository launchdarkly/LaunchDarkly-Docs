...
You can enable secure mode for each environment in your LaunchDarkly account's [Account Settings](https://app.launchdarkly.com/settings/projects). Secure mode is an environment-wide setting. Enabling it requires every request coming from a client-side SDK to contact the back end to evaluate flag variations.
...
You should send the computed secure mode hash for your user as the [`hash` attribute](https://launchdarkly.github.io/js-client-sdk/interfaces/_launchdarkly_js_client_sdk_.ldoptions.html#hash) in the `LDOptions` object during client initialization:
