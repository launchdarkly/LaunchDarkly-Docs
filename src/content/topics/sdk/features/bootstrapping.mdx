...
If your back end passes values to your front end on pageload, you can call your server-side SDK's `allFlagsState` function on pageload and pass the results as a parameter to your front-end initialization code. 
...
function onPageLoad(flags: ld.LDFlagSet) {
  ...
  const user: ld.LDUser = { key: 'aa0ceb'}
  const options: ld.LDOptions = { bootstrap: flags };
...
If you can invoke your back end dynamically, such as in Ruby with a template directory, you can inline the function invocation and request that the SDK return only the client-side flags. 
...
const user: ld.LDUser = { key: 'aa0ceb'}
const options: ld.LDOptions = { bootstrap: 'localStorage' };
const client: ld.LDClient = ld.initialize('YOUR_CLIENT_SIDE_ID', user, options);
```
