...
Number attributes are subject to the same restrictions as number flags. For example, if you define a flag targeting rule where `myAttribute` is one of `0.3`, users with an attribute value of `0.3` may not be correctly targeted because 0.3 can't be exactly represented in binary floating point.
