...
This topic explains the algorithm used to evaluate a feature flag. This document is intended to be used by developers contributing or building server-side SDKs for LaunchDarkly. Client-side and server-side evaluate feature flags differently:
...
Note that variation values are often referenced by their _index_. For example, if a flag has two variations - `true` and `false` - `true` will be variation index 0, and `false` will be variation index 1.
...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell>Integer or string parsed as timestamp</TableCell>
...
You can associate rules with rollouts, which provides flexibility when you assign users to variation buckets.
...
1. Compute the variation bucket for user.
     1. Concatenate the flag's key, the flag's salt, the user's attribute value and the secondary identifier (if any), joined by "." character.
     1. Take the first 15 characters of the SHA1 of the above.
     1. Convert the resulting base 16 integer to a base 10 integer.
     1. Divide the resulting base 10 integer by `0xFFFFFFFFFFFFFFF` (`1152921504606846975`).
  The result of this division is the user's variation bucket number.
