...
This topic explains the algorithm used to evaluate a feature flag. This document is intended to be used by developers contributing or building server-side SDKs for LaunchDarkly. For client-side SDKs, flag evaluation is handled by LaunchDarkly-- there is no need to implement this algorithm. To learn more about the different types of SDK, read [Client-side and server-side SDKs](/sdk/concepts/client-side-server-side).
