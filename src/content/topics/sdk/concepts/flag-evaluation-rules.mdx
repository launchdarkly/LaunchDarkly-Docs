...
As a side effect of the evaluation, the SDKs send events back to LaunchDarkly to report that the evaluation has occurred. The data sent back for each evaluation (including prerequisites) follows the schema described in the [schema reference guide for data export](/integrations/data-export/schema-reference).
...
- Compute the bucket for user
  ...
  - Take the first characters of the sha1 of the above
