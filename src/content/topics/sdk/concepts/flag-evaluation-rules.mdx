...
tags: ['evaluation', 'evaluation context', 'rule', 'server-side', 'sdk']
published: true
---
...
- Server-side SDKs and edge SDKs evaluate feature flags internally using embedded evaluation rules
...
SDKs evaluate flags based on an evaluation context, which may contain one or more specific contexts. The arguments needed to evaluate a flag are the _flag key_, _evaluation context_, and the _default variation value_.

Evaluating a flag for an evaluation context involves the following steps:

1. [Preliminary checks](#preliminary-checks): verify that the flag is on and the supplied evaluation context has a key. If the flag is off, the SDK does not complete the rest of the checks, and serves the default off variation.
2. [Prerequisite checks](#prerequisite-checks): determine whether the flag's prerequisites are met.
3. [Individual targeting checks](#individual-targeting-checks): determine whether any context in the evaluation context matches an individual context targeting rule.
4. [Targeting rule checks](#targeting-rule-checks): determine whether any context matches other targeting rules.
...
- If the context doesn't have a `key` attribute, or if the context key is invalid, return the provided default value with the `ERROR` evaluation reason and the corresponding error code.
...
<Table>
  ...
  <TableBody>
    <TableRow>
      ...
      <TableCell>The context attribute value exactly matches the clause value, including its type.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>endsWith</code>
      ...
      <TableCell>
        Either the clause value is an empty string, or the context attribute value ends with the clause value.
      </TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>String</TableCell>
      <TableCell>The context attribute value starts with the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>matches</code>
      ...
      <TableCell>String</TableCell>
      <TableCell>The clause value, evaluated as a regex, matches the context attribute value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>contains</code>
      ...
      <TableCell>String</TableCell>
      <TableCell>The context attribute value contains the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>lessThan</code>
      ...
      <TableCell>The context attribute value is less than the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>Number</TableCell>
      <TableCell>The context attribute value is less than or equal to the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>Number</TableCell>
      <TableCell>The context attribute value is greater than the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>Number</TableCell>
      <TableCell>The context attribute value is greater than or equal to the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>before</code>
      ...
      <TableCell>The context attribute value less than the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
        <code>after</code>
      ...
      <TableCell>Integer or string parsed as timestamp. This value is a UNIX epoch time in
      milliseconds.</TableCell>
      <TableCell>The context attribute value greater than the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>The context attribute value and the clause value are semantically equivalent.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>String parsed as semantic version</TableCell>
      <TableCell>The context attribute value precedes the clause value.<br/><br/>If negated, corresponds to the context attribute being greater than or equal to the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
      ...
      <TableCell>String parsed as semantic version</TableCell>
      <TableCell>The clause value precedes the context attribute value.<br/><br/>If negated, corresponds to the context attribute being less than or equal to the clause value.</TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
...
Clauses' `segmentMatch` operator allows for more fine-grained control of segments of  contexts. The clause values are segment keys. To determine if this clause is satisfied, retrieve each of the segments identified by those keys and determine if the context is part of any of those segments.
...
1. Get the context's attribute name from the rollout to assign the context. If this is not set, use the context's key.
1. Get the context's attribute value.
1. Get the context's optional secondary identifier.
1. Compute the variation bucket for the context.

   1. Concatenate the flag's key, the flag's salt, the context's attribute value and, if applicable, the secondary identifier. Concatenate them with periods, `.`. If there is a seed present, concatenate seed and the context's attribute value instead.
   1. Copy the first 15 characters of the SHA1 of the above.
   1. Convert the resulting base 16 integer to a base 10 integer.
   1. Divide the resulting base 10 integer by `0xFFFFFFFFFFFFFFF` (`1152921504606846975`). The result of this division is the context's variation bucket number.
...
1. Iterate over the rollout's weighted variations.
   ...
   1. When a context's variation bucket is less than the above sum, return the weighted variation's variation index.
