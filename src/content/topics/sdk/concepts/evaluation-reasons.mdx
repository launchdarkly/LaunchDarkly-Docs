...
To read SDK-specific configuration examples, read [Flag evaluation reasons](/sdk/features/evaluation-reasons).
...
`inExperiment` is an optional attribute on the reason object.  If this is true, it indicates the event is to be included in experimentation analysis.  If `inExperiment` is false, LaunchDarkly does not include this attribute in the reason object.  This is only true for certain SDKs and flags using experiment allocation.
