...
You can also view evaluation reasons on a user's page. To learn how, read [Modifying feature flags for a user](/home/users/users-list#modifying-feature-flags-for-a-user).
...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell>
        The flag is on, but the user did not match any targets or rules, so it returned the value that appears on the flag's <strong>Targeting</strong> tab under "Default rule." The "default rule" is not the same thing as the default value discussed in "Error conditions."
      </TableCell>
    </TableRow>
    <TableRow>
      <TableCell>
...
In this case, the returned flag value is default value that you specified in your code, which is the last parameter of the method you called to evaluate the flag, rather than any value that you specified on your flag's **Targeting** tab. In addition, the variation index will be `null/nil`.
