...
* `feature` and `debug` events which identify which feature flags the SDK is evaluating,
...
* `page view` events which are sent only by JavaScript-based SDKs when pages are visited,
* `click` events which are sent only by JavaScript-based SDKs when elements on pages are clicked,
