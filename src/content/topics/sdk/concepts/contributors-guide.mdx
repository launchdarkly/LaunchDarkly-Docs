...
* `feature` and `debug` events which identify which feature flags are being evaluated,
...
* `page view` events which are sent only by SDKs that are based on the JavaScript SDK when pages are visited,
* `click` events which are sent only by SDKs that are based on the JavaScript SDK when elements on pages are clicked,
* `custom` events which are sent when an application calls the SDK's `track` method,
* `summary` events which summarize a series of `feature` events, and
