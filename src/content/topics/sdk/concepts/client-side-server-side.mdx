...
title: Client-side, server-side, and edge SDKs
description: This topic explains the different security considerations and architectural differences between LaunchDarkly's client-side, server-side, and edge SDKs. It helps you determine which type of SDK to use.
tags: ['sdk', 'client-side', 'server-side', 'key']
published: true
---
...
<Table>
  <TableHeader>
    ...
    <TableHeadCell>These SDKs:</TableHeadCell>
  </TableHeader>
  <TableBody>
    <TableRow>
      ...
      <TableCell>
        <ul>
        <li>are designed for single-user desktop, mobile, and embedded applications.</li>
        <li>are intended to be used in a potentially less secure environment, such as a personal computer or mobile device.</li>
        <li>include mobile SDKs.</li>
        <li>use client-side ID or mobile keys for authorization.</li>
        </ul>
      </TableCell>
    </TableRow>
    <TableRow>
      ...
      <TableCell>
        <ul>
        <li>are designed for multi-user systems.</li>
        <li>are intended to be used in a trusted environment, such as inside a corporate network or on a web server.</li>
        <li>use SDK keys for authorization.</li>
        </ul>
      </TableCell>
    </TableRow>
    <TableRow>
      ...
      <TableCell>
        <ul>
        <li>are designed for edge layers in content delivery networks (CDNs). LaunchDarkly supports [Akamai](/sdk/edge/akamai), [Cloudflare](/sdk/edge/cloudflare), and [Vercel](/sdk/edge/vercel).</li>
        <li>are intended to be used together with a matching [CDN integration](/integrations). The CDN integration allows LaunchDarkly to write to your edge database. Then, you can use the edge SDK to read from that database to provide flag evaluations. This eliminates the need to make a remote call to LaunchDarkly.</li>
        <li>use client-side IDs to associate LaunchDarkly environments with CDN integrations.</li>
        </ul>
      </TableCell>
    </TableRow>
  </TableBody>
...
Edge SDKs must be used in conjunction with an edge provider-specific integration:
  ...
  * You can only use the [Akamai SDK](/sdk/edge/akamai) if you are also using the [Akamai integration](/integrations/akamai).
  * You can only use the [Cloudflare SDK](/sdk/edge/cloudflare) if you are also using the [Cloudflare integration](/integrations/cloudflare).
  * You can only use the [Vercel SDK](/sdk/edge/vercel) if you are also using the [Vercel integration](/integrations/vercel).
...
var ldclient = LDClient.initialize('client-side-id-123abc', context, (options = { allAttributesPrivate: true }))
```
...
var ldclient = LDClient.initialize('client-sideID-123abc', context, (options = { privateAttributeNames: ['email'] }))
```
...
Configure your SDK: [Private attributes](/sdk/features/private-attributes)
...
The SDK key should be kept a secret. If an SDK key is exposed, you can reset it from the **Account settings** page. To learn how, read [Finding and resetting an environment's key](/home/organize/environments#finding-and-resetting-an-environments-sdk-key-mobile-key-or-client-side-id).

### Mobile key

<Callout intent="info">
<CalloutTitle>Making flags available to mobile SDKs</CalloutTitle>
<CalloutDescription>
...
The mobile key does not need to be kept a secret. However, if you wish to reset a mobile key, you can do so from the **Account settings** page. To learn how, read [Finding and resetting an environment's key](/home/organize/environments#finding-and-resetting-an-environments-sdk-key-mobile-key-or-client-side-id).
...
<Table>
  ...
  <TableBody>
    <TableRow>
      <TableCell>
        [.NET (client-side)](/sdk/client-side/dotnet)
      </TableCell>
    ...
    <TableRow>
      <TableCell>
        [C++ (client-side)](/sdk/client-side/c-c--)
      </TableCell>
      <TableCell>Client-side</TableCell>
      <TableCell>Mobile key</TableCell>
    </TableRow>
    ...
    <TableRow>
      <TableCell>
        [Cloudflare](/sdk/edge/cloudflare)
      </TableCell>
      <TableCell>Edge</TableCell>
