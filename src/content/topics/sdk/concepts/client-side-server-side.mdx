  ...
  <TableBody>
    <TableRow>
      ...
      <TableCell>
        ...
        <li>use client-side ID or mobile keys.</li>
        </ul>
      </TableCell>
    </TableRow>
    <TableRow>
...
Edge SDKs must be used in conjunction with an edge provider-specific integration. For example, you can only use the [Cloudflare SDK](/sdk/edge/cloudflare) if you are also using the [Cloudflare integration](/integrations/cloudflare). You can only use the [Vercel SDK](/sdk/edge/vercel) if you are also using the [Vercel integration](/integrations/vercel).

The integration enables flag evaluation within the edge provider by writing the latest flag data from the configured environment directly to your provider store. LaunchDarkly continuously updates the edge provider's cached flag ruleset whenever flag rules change on LaunchDarkly. The edge SDKs do not have to make requests to LaunchDarkly for every evaluated flag. Instead, these SDKs check with the edge provider store to determine which variation a particular context will receive for a given feature flag.
