...
```js
  ...
  var context = {"key": "context-key-123abc"}

  var ldclient = LDClient.initialize("client-side-id-123abc", context);

  var showFeature = ldclient.variation("flag-key-123abc", false);
...
```java
  ...
  boolean showFeature = ldClient.boolVariation("flag-key-123abc", context, false);

```
<!-- cSpell:disable TKTK Check that the above two code samples are correct. The client-side one probably needs to change because key is set automatically for client-side SDKs? SC-178002 cSpell:enable -->
