---
path: /sdk/server-side/erlang/migration-1-to-2
title: Erlang (server-side) SDK 1.x to 2.0 migration guide
description: This topic explains the changes in the server-side Erlang SDK 2.0 release and how to migrate to that version.
published: true
tags: ['Erlang', 'migration', 'sdk', 'server-side', 'context']
---
...
Creating contexts and evaluating flags based on contexts is the primary feature in the Erlang (server-side) SDK 2.0 release. However, the ability to target flags or segments by context, or review context instances that have encountered flags in your application, is only available to members of LaunchDarkly's Early Access Program (EAP). To learn more or join the EAP, read [Contexts](/home/contexts).
...
This topic explains the changes in the Erlang (server-side) SDK 2.0 release and how to adapt code that uses a 1.x version of the [Erlang (server-side) SDK](https://launchdarkly.github.io/erlang-server-sdk/doc/) to use version 2.0 or later.
...
The methods for [evaluating flags](/sdk/features/evaluating) and determining [flag evaluation reasons](/sdk/features/evaluation-reasons) have changed slightly. The 2.0 version of the SDK includes the following changes:
  ...
  * The `variation` and `variation_detail` methods now take a context, rather than a user, as a parameter. To learn more, read [`ldclient:variation/3` API docs](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient.html#variation-3) and [`ldclient:variation_detail/3` API docs](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient.html#variation_detail-3).
...
<CodeSample>
<CSTab label="Impact of context kind">
...
```erlang
Context = ldclient_context:new(<<"user-key-123abc">>),
%% Or as a map
Context = #{key => <<"user-key-123abc">>, kind => <<"user">>}
```
</CSTab>
</CodeSample>
...
In version 2.0, there are two scopes for which you can mark attributes as private:
  ...
  * Within a particular context or context kind. You might use this if you want an "email" attribute to be private in a user context, but not in an organization context.
  ...
  ```erlang
  ldclient:start_instance("your-sdk-key", #{private_attributes => all}).
  ```
  </CSTab>
  <CSTab label="2.0 syntax, two attributes marked private">

  ```erlang
  ldclient:start_instance("your-sdk-key", #{private_attributes => [<<"email">>, <<"address">>]}).
  ```
  </CSTab>
  </CodeSample>

To learn more, read [`ldclient_context:set_private_attributes/2` API docs](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient_context.html#set_private_attributes-2) and [`ldclient_config` API docs](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient_config.html).
...
The `private_attribute_names` attribute that existed in a user object in version 1.x has been renamed to `private_attribute` in version 2.0. To learn more, read [`ldclient_context` API docs](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient_context.html).
...
To learn more, read [`ldclient_context:new_multi_from/1 API docs`](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient_context.html#new_multi_from-1).

## Understanding changes to configuration options

In the 2.0 version of the SDK, several configuration options have changed:

  * The `private_attributes` configuration option now applies to all contexts of any context kind. To learn more, read [Understanding changes to private attributes](#understanding-changes-to-private-attributes).
...
To learn more, read [`ldclient_config` API docs](https://launchdarkly.github.io/erlang-server-sdk/doc/ldclient_config.html).
