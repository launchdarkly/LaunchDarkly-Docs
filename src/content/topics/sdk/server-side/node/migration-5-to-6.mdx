...
* `yaml`: The SDK no longer uses the `yaml` package by default. This only affects [`FileDataSource`](https://launchdarkly.github.io/node-server-sdk/modules/_launchdarkly_node_server_sdk_.html#FileDataSource). To learn more, read "Understanding changes to the file data source".
