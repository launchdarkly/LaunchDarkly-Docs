...
title: 'Server-side Node.js SDK 5.x to 6.0 migration guide'
...
* `redis`: The Redis integration is no longer included in the SDK package. It is now a separate package. To learn more, see "Understanding changes to the Redis integration".
* `winston`: The SDK no longer uses the `winston` package by default. You can still use `winston`. To learn more, see "Understanding changes to logging".
* `yaml`: The SDK no longer uses the `yaml` package by default. This only affects [`FileDataSource`](https://launchdarkly.github.io/node-server-sdk/index.html#filedatasource). To learn more, see "Understanding changes to the file data source".
...
```bash
npm install --save launchdarkly-node-server-sdk-redis
```
...
The SDK also has a new option to allow configuring a simple logger without using `winston`. To learn more, read [`basicLogger`](https://launchdarkly.github.io/node-server-sdk/index.html#basicLogger).

## Understanding changes to the file data source

The [`FileDataSource`](https://launchdarkly.github.io/node-server-sdk/index.html#filedatasource) component allows you to load feature flag data from files. The file data can be either JSON or YAML.
