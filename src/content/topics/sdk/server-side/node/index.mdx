...
This SDK is intended for use in *multi-user* Node.js server applications. If you want to set up LaunchDarkly in JavaScript in a browser environment, read [JavaScript SDK](/sdk/client-side/javascript). If you're creating a *client-side* Node application, read [Node SDK](/sdk/client-side/node-js). If you're creating a desktop application in Electron, read [Electron SDK](/sdk/client-side/electron).
...
The client will emit a `ready` event when you initialize it and it can serve feature flags.

You can check which variation a particular user should receive for a given feature flag using `client`. The `ready` event is only emitted once, when the client first initializes. In a production application you should place your `client.variation` code so that it is invoked as desired.
