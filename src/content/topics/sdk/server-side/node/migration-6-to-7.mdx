...
<ul>
<li>The fields in your user object must be <code>LDUser</code> fields, not <code>LDContext</code> fields. For example, to mark an attribute as private, you must use <code>privateAttributeNames</code> in the user object, not <code>_meta.privateAttributes</code> as you would for a context object. Nested JSON, such as is used in <code>_meta.privateAttributes</code>, is not supported by older versions of the SDK. To learn more, read [Understanding changes to private attributes](#understanding-changes-to-private-attributes).</li>
