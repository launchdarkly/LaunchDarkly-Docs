...
description: 'This reference guide documents all of the methods available in the Python SDK, and explains how these methods work.'
published: true
---

This reference guide documents all of the methods available in the Python SDK, and explains how these methods work. LaunchDarkly's SDKs are open source. The source is available in its [GitHub repository](https://github.com/launchdarkly/python-server-sdk) and its generated [API reference](https://launchdarkly-python-sdk.readthedocs.io/).

You can also use this SDK to clone and run a [sample application](https://github.com/launchdarkly/hello-python). For an example of using the Python SDK inside an R application, clone and run [hello-r](https://github.com/launchdarkly/hello-r).
...
The first step is to install the LaunchDarkly SDK as a dependency in your application using your application's dependency manager. If you want to depend on a specific version, refer to the [SDK releases page](https://github.com/launchdarkly/python-server-sdk/releases) to identify the latest version.
...
Using `client`, you can check which variation a particular will should receive for a given feature flag:
