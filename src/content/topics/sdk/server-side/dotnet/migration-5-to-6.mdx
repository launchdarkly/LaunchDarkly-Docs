...
It also has an adapter system for connecting it to other logging frameworks. If you use .NET Core, there is a built-in adapter that sends log output to the .NET Core logging framework (`Microsoft.Extensions.Logging`):
...
* You can tell the SDK to notify you whenever a feature flag's configuration has changed with `LdClient.FlagTracker`. See [Subscribing to flag changes](/sdk/features/flag-changes#net).
