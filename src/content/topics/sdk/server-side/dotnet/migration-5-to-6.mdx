...
title: '.NET SDK 5.x to 6.0 migration guide'
description: 'This topic explains the API changes made in the .NET SDK 6 release and how to migrate to that version.'
published: true
---

<Callout intent="info">
  <CalloutTitle>This migration guide is for a prerelease version of the .NET SDK</CalloutTitle>
  <CalloutDescription>

.NET SDK 6.0 is not yet generally available. This topic describes the `6.0.0-beta.1` prerelease version. This is a living document that may change if we make additional modifications to the SDK before the final release.
...
* Any other runtime that supports the .NET Standard 2.0 API. However, even though Xamarin is compatible with .NET Standard 2.0, this SDK is not intended to be used with Xamarin. Use the [Xamarin SDK](/sdk/client-side/xamarin) instead.

LaunchDarkly no longer supports older .NET target frameworks, including .NET Standard 1.6, as is documented in our [End of Life policy](https://launchdarkly.com/policies/end-of-life-policy/).
...
The following examples use the [Redis integration](https://github.com/launchdarkly/dotnet-server-redis).
...
The new package provides some basic built-in logging implementations suitable for simple applications, such as logging to the console. Here is an example of configuring the .NET SDK to send log output to the console, and to enable only log messages of `Warn` level or higher.
...
It also has an adapter system for connecting it to other logging frameworks. If you are using .NET Core, there is a built-in adapter that sends log output to the .NET Core logging framework (`Microsoft.Extensions.Logging`):
...
In version 5.14.0, we added a new configuration syntax. Starting in version 6.0, the new syntax is the only way to configure these.
...
```csharp
// 5.x model: proxy mode with streaming
...
// 5.x model: proxy mode with polling
var relayUri = new Uri("http://my-relay-host:8000");
var config =  Configuration.Builder("my-sdk-key")
    .IsStreamingEnabled(false)
    .BaseUri(relayUri)
...
// 5.x model: daemon mode with a Redis database
var config =  Configuration.Builder("my-sdk-key")
    .FeatureStore(
...
```csharp
// 6.0 model: proxy mode with streaming
var relayUri = new Uri("http://my-relay-host:8000");
var config =  Configuration.Builder("my-sdk-key")
...
Third, if you prefer to use the popular [Newtonsoft Json.NET](https://www.newtonsoft.com/json) framework, see below.
...
Most applications use either the default in-memory storage or one of the [database integrations](/sdk/concepts/feature-store) LaunchDarkly provides (Consul, DynamoDB, Redis), but the data store interface (formerly called "feature store") has always been public so developers can write their own integrations.
...
To learn more, read the API documentation for the `IPersistentDataStore` and `IDataSource` interfaces. You may also want to review the source code for one of the LaunchDarkly database integrations, such as [Redis](http://github.com/launchdarkly/dotnet-server-sdk-redis), to see how it changed from the previous major version.
...
* You can tell the SDK to notify you whenever a feature flag's configuration has changed, using `LdClient.FlagTracker`.
