...
Once the SDK is installed and imported, you'll want to create a single, shared instance of [`LdClient`](https://launchdarkly.github.io/dotnet-server-sdk/api/LaunchDarkly.Sdk.Server.LdClient.html). You should specify your SDK key here so that your application will be authorized to connect to LaunchDarkly and for your application and environment.
...
.NET, the DNS cache retains IP addresses for two minutes by default. If you notice intermittent connection
failures that always resolve in two minutes, you may want to change this setting to a lower value as described in [Microsoft's documentation](https://docs.microsoft.com/en-us/dotnet/api/system.NET.servicepointmanager.dnsrefreshtimeout?view=netframework-4.7.2).
...
* [Storing data](/sdk/features/storing-data#cc-server-side#net)
