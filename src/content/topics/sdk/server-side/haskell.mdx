---
path: '/sdk/server-side/haskell'
title: 'Haskell SDK reference'
description: 'This topic explains all of the methods available in the server-side Haskell SDK, with detail about how they work.'
published: true
---

## Overview

This reference guide documents all of the methods available in the Haskell SDK, and explains in detail how these methods work. LaunchDarkly's SDKs are open source. To learn more, visit our [Haskell SDK GitHub repository](https://github.com/launchdarkly/haskell-server-sdk). The online [API docs](https://launchdarkly.github.io/haskell-server-sdk/) contain the programmatic definitions of every type and method. You can also try this SDK out by cloning and running a [sample application](https://github.com/launchdarkly/hello-haskell-server).

## Getting started

Follow the steps below to get started using the LaunchDarkly SDK in your Haskell application.

The root module of the SDK `LaunchDarkly.Server` re-exports the entire project. That's all that you need to start.

Here is an example:

<CodeTabs
  defaultValue="haskell"
  values={[
    { label: 'Haskell', value: 'haskell', },
  ]
}>
<CodeTabItem value="haskell">

```haskell
import LaunchDarkly.Server
```

</CodeTabItem>
</CodeTabs>

After you import and install the SDK, create a single shared instance of `Client`. Specify your SDK key to authorize your application to connect to LaunchDarkly for a particular environment.

Here's how:

<CodeTabs
  defaultValue="haskell"
  values={[
    { label: 'Haskell', value: 'haskell', },
  ]
}>
<CodeTabItem value="haskell">

```haskell
client :: IO Client
client = makeClient $ makeConfig "YOUR_SDK_KEY"
```

</CodeTabItem>
</CodeTabs>

<Callout intent="alert">
<CalloutTitle>Client must be a singleton</CalloutTitle>
<CalloutDescription>

Make `client` a singleton. The client instance maintains an internal state that allows us to serve feature flags without making any remote requests.

</CalloutDescription>
</Callout>

Using `client`, you can check which variation a particular user should receive for a given feature flag:

<CodeTabs
  defaultValue="haskell"
  values={[
    { label: 'Haskell', value: 'haskell', },
  ]
}>
<CodeTabItem value="haskell">

```haskell
boolVariation client "YOUR_FLAG_KEY" (makeUser "abc") False
```

</CodeTabItem>
</CodeTabs>

Lastly, shut down the client when your application terminates. To learn more, read [Shutting down](/sdk/features/shutdown#haskell).

## Supported features

This SDK supports the following features:

* [Aliasing users](/sdk/features/aliasing-users#haskell)
* [Configuration](/sdk/features/config#haskell)
* [Evaluating flags](/sdk/features/evaluating#haskell)
* [Flag evaluation reasons](/sdk/features/evaluation-reasons#haskell)
* [Flushing events](/sdk/features/flush#haskell)
* [Getting all flags](/sdk/features/all-flags#haskell)
* [Identifying and changing users](/sdk/features/identify#haskell)
* [Offline mode](/sdk/features/offline-mode#haskell)
* [Sending custom events](/sdk/features/events#haskell)
* [Shutting down](/sdk/features/shutdown#haskell)
* [User configuration](/sdk/features/user-config#haskell)
