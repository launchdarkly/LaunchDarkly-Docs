...
Go SDK 5.0 is not yet generally available. This topic describes the `v5.0.0-beta.3` pre-release version. This is a living document that may change if there are additional modifications to the SDK before the final release.
...
The Consul, DynamoDB, and Redis integrations are no longer bundled in the main SDK project. Instead, they are maintained in separate repositories: [`github.com/launchdarkly/go-server-sdk-redis`](https://github.com/launchdarkly/go-server-sdk-redis), [`github.com/launchdarkly/go-server-sdk-dynamodb`](https://github.com/launchdarkly/go-server-sdk-dynamodb), and [`github.com/launchdarkly/ldconsul`](https://github.com/launchdarkly/go-server-sdk-consul).

The new import paths for those packages are the same as the repository locations: `github.com/launchdarkly/go-server-sdk-redis`, etc.
