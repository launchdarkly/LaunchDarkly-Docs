...
<Callout intent="info">
  <CalloutTitle>This migration guide is for a pre-release version of the Go SDK</CalloutTitle>
  <CalloutDescription>

Go SDK 5.0 is not yet generally available. This topic describes the `v5.0.0-beta.1` pre-release version. This is a living document that may change if there are additional modifications to the SDK before the final release.
...
The minimum Go version for LaunchDarkly SDK 5.0 is 1.13. 
...
* [`gopkg.in/launchdarkly/go-sdk-server.v5`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5)
  ...
  * [`/ldcomponents`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldcomponents): configuration builders for specific areas of SDK functionality, such as configuring streaming options or logging options. See [Understanding changes to SDK configuration](#understanding-changes-to-sdk-configuration).
  * [`/ldconsul`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldconsul), [`/lddynamodb`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/lddynamodb), [`/ldredis`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldredis), [`ldfiledata`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldfiledata): add-ons such as database integrations which already existed in the 4.x SDK, but have somewhat different configuration syntax now. Note that `ldredis` was formerly called `redis`.
...
2. The `VariationIndex` property of `EvaluationDetail` is now an `int`, not `*int`. In cases where it previously would have been `nil`, it is now `-1`.
...
The Consul, DynamoDB, and Redis integrations are still included in the SDK. However, the name of the package for the Redis integration is now [`ldredis`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldredis) instead of `redis`. This is consistent with the other two, which are [`ldconsul`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/ldconsul) and [`lddynamodb`](https://pkg.go.dev/gopkg.in/launchdarkly/go-server-sdk.v5/lddynamodb).

The configuration syntax has changed. To learn more about configuration details, read [Understanding changes to the data store](#understanding-changes-to-the-data-store).
