...
```js
ldClient.on('update:YOUR_FLAG_KEY', (oldValue, newValue) => {
...
A custom logger may be passed to the SDK by the configurable `logger` property. The logger must suppot `error`, `warn`, `info`, and `debug` levels. Note that Winston's syntax for instantiating and configuring loggers changed between versions 2.x and 3.x. Be sure to declare a dependency on a specific Winston version if you run into errors using the transitive dependency.
