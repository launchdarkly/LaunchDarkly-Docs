...
It's important to make this a singleton-- internally, the client instance maintains internal state that allows us to Íserve feature flags without making any remote requests. **Be sure that you're not instantiating a new client with every request.**
...
To learn more, read the [API documentation](https://www.rubydoc.info/gems/launchdarkly-server-sdk/LaunchDarkly/Integrations) and ["Using a persistent feature store"](/home/advanced/feature-store).
