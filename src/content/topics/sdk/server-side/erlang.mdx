...
```
{deps, [
	{eld, {git, "https://github.com/launchdarkly/erlang-server-sdk", {tag, "1.0.0-beta1"}}}
]}.
```
...
```
Flag = eld:variation(<<"YOUR_FLAG_KEY">>, #{key => <<"123">>}, false)
```
...
```
% Specify options
eld:start_instance("YOUR_SDK_KEY", #{stream => false})
...
```
Flag = eld:variation_detail(<<"my-bool-key">>, #{key => <<"aa0ceb">>}, false)
```
...
```
eld:all_flags_state(#{key => <<"aa0ceb">>})
```
...
```
eld:track(<<"YOUR-METRIC-KEY">>, #{key => <<"aa0ceb">>}, #{data => <<"example">>})
```
...
```
eld:track_with_metric(<<"YOUR-METRIC-KEY">>, #{key => <<"aa0ceb">>}, #{data => <<"example">>}, 0.5)
```
...
```
eld:start_instance("YOUR_SDK_KEY", #{offline => true})
```
