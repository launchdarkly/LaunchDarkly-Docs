...
Java SDK 5.0 is not yet generally available. This topic describes the `5.0.0-rc1` prerelease version. This is a living document that may change if there are additional modifications to the SDK before the final release.
...
### Undersatnding changes to data source methods
...
## Using Gson or Jackson for JSON conversion

[Gson](https://github.com/google/gson) and [Jackson](https://github.com/FasterXML/jackson) are two commonly used JSON frameworks for Java. If you are not using either of them, you don't need to read this section.

You might want to convert LaunchDarkly SDK classes such as `LDUser`, `LDValue`, or `FeatureFlagsState` into JSON if you are passing them to front-end JavaScript code. Less commonly, you might want to convert them from JSON. The SDK provides a way to do both.
...
As long as you have registered `LDGson.typeAdapters()` in the Gson instance you're using, `toJson()` and `fromJson()` will work as expected with LaunchDarkly types. You can also use the serialize and deserialize methods in `com.launchdarkly.sdk.json.JsonSerialization` to quickly convert things to or from JSON strings without needing a Gson instance.
...
* You can tell the SDK to notify you whenever a feature flag's configuration has changed in any way, using `FlagChangeListener` and `LDClientInterface.registerFlagChangeListener()`.
* Or, you can tell the SDK to notify you only if the value of a flag for some particular User has changed, using `FlagValueChangeListener` and `Components.flagValueMonitoringListener()`.
