...
The Java SDK is intended for use in server environments only, and should not be used in mobile devices. If you want to use LaunchDarkly in an Android application, read the [Android SDK reference](/sdk/client-side/android).

For information about how to choose an SDK, read [Client-side and server-side SDKs](/sdk/concepts/client-side-server-side).
...
It's important to make this a singleton. Internally, the client instance maintains internal state that allows LaunchDarkly to serve feature flags without making any remote requests. Do not instantiate a new client with every request.
