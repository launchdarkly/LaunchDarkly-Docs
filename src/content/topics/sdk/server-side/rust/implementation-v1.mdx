...
<CodeTabs
  ...
  values={[
    { label: 'Rust', value: 'variation', },
  ]
}>
<CodeTabItem value="variation">

```rust
let value = client.bool_variation(&context, "YOUR-FLAG-KEY", false);
```
</CodeTabItem>
</CodeTabs>
...
<CodeTabs
  ...
  values={[
    ...
    { label: 'Beta version, user with key', value: 'user', },
  ]
}>
<CodeTabItem value="single-context">

```rust
let context = ContextBuilder::new("context-key").build()?;
```
</CodeTabItem>
<CodeTabItem value="user">

```rust
let user1 = User::with_key("user-key").build();
```
</CodeTabItem>
</CodeTabs>
...
<CodeTabs
  defaultValue="single-context"
  values={[
    { label: 'Single context builder', value: 'single-context', },
  ]
}>
<CodeTabItem value="single-context">

```rust
let context = ContextBuilder::new("context-key")
    .kind("organization")
    .build()?;
```
</CodeTabItem>
...
```rust
let user_context = ContextBuilder::new("user-key").build()?;
client.identify(user_context.clone());

let device_context = ContextBuilder::new("device-key").kind("device").build()?;
...
<CodeTabs
  ...
  values={[
    ...
    { label: 'Beta version, user with several attributes', value: 'user', },
  ]
}>
...
```rust
let context = ContextBuilder::new("the-key")
    .set_value(
        "address",
...
<CodeTabs
  defaultValue="rust"
  values={[
    { label: 'Two attributes marked private for all contexts', value: 'rust', },
  ]
}>
<CodeTabItem value="rust">

```rust
let config_builder = ConfigBuilder::new("sdk-key");
...
```rust
ContextBuilder::new("the-key").add_private_attribute(Reference::new("/address/street"))
```
</CodeTabItem>
</CodeTabs>
