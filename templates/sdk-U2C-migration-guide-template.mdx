...
<CodeSample>
<CSTab label="Y.0 syntax, single context with key">
...
<CodeSample>
<CSTab label="Y.0 syntax, multi-context">
...
You can define additional attributes for a context by passing in a name and value for each. Additional attributes can be any JSON type, including boolean, number, string, array, or object. <!-- include this sentence if the old version had custom attrs in a custom object --> In version Y.0, you do not need to add custom attributes within a `custom` object.

Here's how to construct a context with additional attributes, as compared with constructing a similar user:

<CodeSample>
<CSTab label="X.x syntax, user with attributes">

```LANGUAGE-NAME
TKTK - example building a user in the previous version of the SDK, with a key, name, and email
```
</CSTab>
<CSTab label="Y.0 syntax, context with attributes">
...
<CodeSample>
<CSTab label="Y.0 syntax, context with object attributes">
...
In the first example, all attributes are marked private for all contexts. <!-- Include the following sentence only for server-side SDKs. --> Only the context key and kind are sent to LaunchDarkly. In the second example, the "email" and "address" attributes are private for all contexts:

<CodeSample>
<CSTab label="Y.0 syntax, all attributes marked private">

```LANGUAGE-NAME
TKTK - example setting the the `allAttributesPrivate` SDK configuration option.
```
</CSTab>
<CSTab label="Y.0 syntax, two attributes marked private">
...
<CodeSample>
<CSTab label="Y.0 syntax, attribute marked private for one context">
...
<!-- The above paragraph is relevant for SDKs that do not have a context builder, where they actually have to write out what the context would look like in JSON. That's true for instance of the Node.js SDK, but not Go, Java, .NET, or Ruby (Ruby does not currently have a user builder, but it will have a context builder). -->
...
<CodeSample>
<CSTab label="Y.0 syntax, setting device and OS">
...
<CodeSample>
<CSTab label="Y.0 syntax, setting the device ID">
...
<CodeSample>
<CSTab label="Y.0 syntax, associating two contexts">
