...
The Y.0 version of this SDK lets you use contexts. When you migrate from version X.x, you must replace every instance of a user with a context. <!-- Make sure to update the type names as applicable. --> For the Y.0 version of the LANGUAGE-NAME SDK, this means changing each use of `LDUser` to `LDContext`.
...
<CodeTabs
  ...
  values={[
    { label: 'LANGUAGE NAME', value: 'variation', },
  ]
}>
...
<CodeTabs
  ...
  values={[
    ...
    { label: 'Version X.x, user with key', value: 'user', },
  ]
}>
...
<CodeTabs
  ...
  values={[
    ...
    { label: 'Version X.x, user with several attributes', value: 'user', },
  ]
}>
...
<CodeTabs
  defaultValue="device"
  values={[
    { label: 'Setting device and operating system', value: 'device', },
  ]
}>
...
<CodeTabs
  ...
  values={[
    { label: 'Setting the device ID', value: 'deviceid', },
  ]
}>
